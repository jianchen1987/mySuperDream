...
- (void)showBadgeWithTopMagin:(NSDictionary *)info {
    if (![info isKindOfClass:NSDictionary.class])
        return;
    if (![info.allKeys containsObject:WMZPageKeyBadge])
        return;
    ...
    if ([value isKindOfClass:NSString.class] && [value length] > 1) {
        ...
        BOOL wHideRedCircle = self.param.wHideRedCircle;
        self.param.wHideRedCircle = YES;
        [self hidenBadge];
        self.param.wHideRedCircle = wHideRedCircle;
        return;
    }
    if ([value isEqual:@(-1)]) {
        self.badge.text = @"";
        ...
        self.badge.layer.backgroundColor = HDAppTheme.color.gn_mainColor.CGColor;
        self.badge.textColor = UIColor.whiteColor;
        self.badge.text = [NSString stringWithFormat:@"%@", info[WMZPageKeyBadge]];
        self.badge.pageEdgeInsets = UIEdgeInsetsMake(0, kRealWidth(5), 0, kRealWidth(5));
        CGSize size = [self boundingRectWithSize:self.badge.text Font:self.badge.font Size:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)];
        CGRect rect = CGRectZero;
        rect.size.width = MAX(kRealWidth(18), size.width + self.badge.pageEdgeInsets.right + self.badge.pageEdgeInsets.left);
        rect.size.height = kRealWidth(18);
        rect.origin.y = kRealWidth(8);
        self.badge.frame = rect;
        self.badge.layer.cornerRadius = kRealWidth(9);
    }
    self.badge.alpha = 1;
...
- (void)showWMBadgeWithTopMagin:(NSDictionary *)info {
    if (![info isKindOfClass:NSDictionary.class])
        return;
    if (![info.allKeys containsObject:WMZPageKeyBadge])
        return;
    ...
    if ([value isEqual:@(-1)]) {
        ...
        self.badge.text = [NSString stringWithFormat:@"%@", info[WMZPageKeyBadge]];
        CGRect rect = CGRectZero;
        rect.size.width = kRealWidth(16);
        rect.size.height = kRealWidth(16);
        rect.origin.y = kRealWidth(4);
        rect.origin.x = CGRectGetMaxX(self.imageView.frame) - kRealWidth(8);
        self.badge.frame = rect;
        self.badge.layer.cornerRadius = kRealWidth(8);
    }
    self.badge.alpha = 1;
    [self addSubview:self.badge];
    [self bringSubviewToFront:self.badge];
...
- (CGSize)boundingRectWithSize:(NSString *)txt Font:(UIFont *)font Size:(CGSize)size {
    return [txt boundingRectWithSize:size options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading
...
- (CGSize)maxSize {
    if (!_maxSize.width || !_maxSize.height) {
        _maxSize = [self boundingRectWithSize:self.titleLabel.text Font:self.titleLabel.font Size:CGSizeMake(CGFLOAT_MAX, self.param.wMenuPosition == PageMenuPositionNavi ? 35 : CGFLOAT_MAX)];
    }
...
- (WMZPageLabel *)badge {
    if (!_badge) {
        ...
        _badge.backgroundColor = HDAppTheme.color.gn_mainColor;
        _badge.layer.cornerRadius = pointWidth / 2;
        _badge.font = [HDAppTheme.font gn_ForSize:12];
...
+ (instancetype)bm_colorGradientChangeWithSize:(CGSize)size direction:(PageGradientChangeDirection)direction startColor:(UIColor *)startcolor endColor:(UIColor *)endColor {
