...
- (id)forwardingTargetForSelector:(SEL)aSelector {
    if ([procotolMethodArr indexOfObject:NSStringFromSelector(aSelector)] != NSNotFound && [self.pageView respondsToSelector:aSelector]) {
...
- (void)setParam:(WMZPageParam *)param {
    ...
    if (self.param.wMenuPosition == PageMenuPositionNavi || self.param.wMenuPosition == PageMenuPositionBottom || !self.param.wLazyLoading) {
        [self showData];
    } else {
...
- (void)change:(NSNotification *)notification {
    if (!self.param.wDeviceChange)
        return;
    ([UIDevice currentDevice].orientation == UIDeviceOrientationLandscapeLeft || [UIDevice currentDevice].orientation == UIDeviceOrientationLandscapeRight) ? [self changeLeft:YES] :
                                                                                                                                                              [self changeLeft:NO];
}
