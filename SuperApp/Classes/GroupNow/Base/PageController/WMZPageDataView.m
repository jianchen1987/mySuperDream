...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    if ([NSStringFromClass([gestureRecognizer.view class]) isEqualToString:@"WMZPageDataView"] && [NSStringFromClass([otherGestureRecognizer.view class]) isEqualToString:@"WMZPageDataView"] &&
        ...
        if (firstView.level - secondView.level == -1) {
            ...
            if (firstView.contentOffset.x <= 0) {
                if (firstView.left && secondView.currentIndex > 0)
                    return YES;
                if (!firstView.left && firstView.currentIndex == (firstView.totalCount - 1))
                    return YES;
            } else if (firstView.contentOffset.x >= self.pageWidth * (firstView.totalCount - 1)) {
                if (firstView.left && firstView.currentIndex == 0 && secondView.currentIndex > 0)
                    return YES;
                if (!firstView.left && secondView.currentIndex < (secondView.totalCount - 1))
                    return YES;
            }
        }
    }
    ...
    if ([NSStringFromClass([gestureRecognizer class]) isEqualToString:@"UIScrollViewPanGestureRecognizer"] && self.respondGuestureType != PagePopNone) {
        ...
        if (point.x >= 0 && (self.panGestureRecognizer.state == UIGestureRecognizerStateBegan || self.panGestureRecognizer.state == UIGestureRecognizerStateChanged)) {
