...
- (void)loadUI:(CGFloat)width clear:(BOOL)clear {
    ...
    [self setUp:clear];
    ///适配横竖屏
    if (self.param.wDeviceChange && self.dataView.subviews.count && lastPageWidth) {
...
- (void)setUp:(BOOL)clear {
    ...
    if (self.param.wMenuAddSubView) {
        ...
        if ([insertView isKindOfClass:UIView.class]) {
            ...
            self.param.wMenuInsets
                = UIEdgeInsetsMake(self.param.wMenuInsets.top, self.param.wMenuInsets.left, self.param.wMenuInsets.bottom + CGRectGetMaxY(self.insertView.frame), self.param.wMenuInsets.right);
        }
    }
    /// 菜单栏
    ...
    if (!btn.tapType) {
        ...
        if (self.currentTitleIndex == NSNotFound) {
            ...
            if (self.param.wEventEndTransferController)
                self.param.wEventEndTransferController(nil, (id)newVC, 0, index);
...
- (void)scrollViewDidScroll:(WMZPageDataView *)scrollView {
    ...
    if (scrollView.popGuestureOffset >= 0 && self.param.wRespondGuestureType != PagePopNone) {
        [scrollView setContentOffset:CGPointMake(scrollView.popGuestureOffset, 0) animated:NO];
        return;
    }
...
- (void)scrollViewDidEndDragging:(WMZPageDataView *)scrollView willDecelerate:(BOOL)decelerate {
    if (scrollView != self.dataView)
        return;
    if (self.dataView.popGuestureOffset != -1)
        self.dataView.popGuestureOffset = -1;
    if (!decelerate) {
...
- (void)lifeCycleManage:(UIScrollView *)scrollView {
    ...
    if (self.currentTitleIndex != currentIndex && self.param.wMenuFollowSliding) {
...
- (void)beginAppearanceTransitionWithIndex:(NSInteger)index withOldIndex:(NSInteger)old {
    ...
    if (!newVC || !oldVC || (index == old) || (oldVC == newVC))
        return;
    ...
    if (self.param.wEventBeganTransferController)
        self.param.wEventBeganTransferController((id)oldVC, (id)newVC, old, index);
}

- (void)addChildVC:(NSInteger)index VC:(UIResponder *)newReponder {
    if (!newReponder)
        return;
    CGRect frame = CGRectMake(index * self.pageWidth, 0, self.pageWidth, self.dataView.frame.size.height);
...
- (void)endAppearanceTransitionWithIndex:(NSInteger)index withOldIndex:(NSInteger)old isFlag:(BOOL)flag {
    ...
    if (!newResponer || !oldResponer || (index == old))
        return;
    if (self.currentTitleIndex == self.nextPageIndex) {
        ...
        if (self.param.wEventEndTransferController)
            self.param.wEventEndTransferController((id)oldResponer, (id)newResponer, old, index);
        ...
        if (self.param.wEventEndTransferController)
            self.param.wEventEndTransferController((id)newResponer, (id)oldResponer, index, old);
        self.currentVC = (id)oldResponer;
        self.currentTitleIndex = old;
        if (self.loopDelegate && [self.loopDelegate respondsToSelector:@selector(setUpSuspension:index:end:)])
            [self.loopDelegate setUpSuspension:oldResponer index:old end:YES];
    }
...
- (void)viewProtocolAction:(NSString *)tag view:(id)view {
    if (!tag && ![tag isKindOfClass:NSString.class])
        return;
    if (![view conformsToProtocol:@protocol(WMZPageProtocol)])
        return;
    if ([tag isEqualToString:@"wa"] && [view respondsToSelector:@selector(pageViewWillAppear)])
        [view pageViewWillAppear];
    if ([tag isEqualToString:@"da"] && [view respondsToSelector:@selector(pageViewDidAppear)])
        [view pageViewDidAppear];
    if ([tag isEqualToString:@"wd"] && [view respondsToSelector:@selector(pageViewWillDisappear)])
        [view pageViewWillDisappear];
    if ([tag isEqualToString:@"dd"] && [view respondsToSelector:@selector(pageViewDidDisappear)])
        [view pageViewDidDisappear];
}
...
- (NSDictionary *)frameInfo {
    ...
    _frameInfo = @{
        ...
        @(PageMenuPositionRight):
            [NSValue valueWithCGRect:CGRectMake(self.pageWidth - self.param.wMenuWidth, menuCellMarginY, self.param.wMenuWidth, self.param.wMenuHeight + self.param.wMenuInsets.bottom)],
        @(PageMenuPositionCenter):
            [NSValue valueWithCGRect:CGRectMake((self.pageWidth - self.param.wMenuWidth) / 2, menuCellMarginY, self.param.wMenuWidth, self.param.wMenuHeight + self.param.wMenuInsets.bottom)],
        @(PageMenuPositionNavi):
            [NSValue valueWithCGRect:CGRectMake((self.pageWidth - self.param.wMenuWidth) / 2, menuCellMarginY, self.param.wMenuWidth, self.param.wMenuHeight + self.param.wMenuInsets.bottom)],
        @(PageMenuPositionBottom): [NSValue valueWithCGRect:CGRectMake(menuCellMarginY,
                                                                       self.pageWidth,
                                                                       self.param.wMenuWidth,
                                                                       (PageIsIphoneX ? (self.param.wMenuHeight + 34) : self.param.wMenuHeight) + self.param.wMenuInsets.bottom)],
    };
    return _frameInfo;
}
