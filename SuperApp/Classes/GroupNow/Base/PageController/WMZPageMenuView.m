...
- (void)UI {
    ...
    if (temp)
        [self resetMainViewContenSize:temp];
    /// 最底部固定线
...
- (void)resetMainViewContenSize:(WMZPageNaviBtn *)btn {
    ...
    if (self.contentSize.width < self.frame.size.width && self.param.wMenuPosition == PageMenuPositionCenter && self.param.wMenuWidth == PageVCWidth) {
...
- (void)setUpIndicator {
    ...
    self.lineView.hidden = (self.param.wMenuAnimal == PageTitleMenuNone || self.param.wMenuAnimal == PageTitleMenuTouTiao || self.param.wMenuAnimal == PageTitleMenuCircleBg
...
- (void)setUpFixRightBtn:(WMZPageNaviBtn *)temp {
    if (self.param.wMenuFixRightData) {
        ...
        for (int i = ((int)fixData.count - 1); i >= 0; i--) {
            ...
            CGFloat originY =
                [self getTitleData:info key:WMZPageKeyTitleMarginY] ? [[self getTitleData:info key:WMZPageKeyTitleMarginY] floatValue] : (temp.frame.origin.y + self.param.wMenuInsets.top);
            ...
            if (text) {
                if ([text isKindOfClass:NSString.class])
                    ...
                    [fixBtn setAttributedTitle:text forState:UIControlStateNormal];
            ...
            if (selectText) {
                ...
                else if ([selectText isKindOfClass:NSAttributedString.class])
                    [fixBtn setAttributedTitle:selectText forState:UIControlStateSelected];
            ...
            if (self.param.wMenuFixShadow) {
                [fixBtn.layer.sublayers enumerateObjectsUsingBlock:^(__kindof CALayer *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
                    if ([obj isKindOfClass:CAGradientLayer.class]) {
                        [obj removeFromSuperlayer];
                    }
                }];
                CAGradientLayer *gl = [CAGradientLayer layer];
                gl.frame = CGRectMake(0, 0, 1, menuFixWidth);
                ;
...
- (void)setPropertiesWithBtn:(WMZPageNaviBtn *)btn withIndex:(NSInteger)i withTemp:(WMZPageNaviBtn *)temp {
    ...
    if (name) {
        ...
        else if ([name isKindOfClass:NSAttributedString.class])
            [btn setAttributedTitle:name forState:UIControlStateNormal];
        btn.normalText = name;
    }
    ...
    if (selectText) {
        if ([selectText isKindOfClass:NSString.class])
            ...
            [btn setAttributedTitle:selectText forState:UIControlStateSelected];
    ...
    if (!titleBackground)
        titleBackground = self.param.wMenuTitleBackground ?: UIColor.clearColor;
    if (titleBackground)
    ...
    if (onlyClick)
        btn.tapType = PageBTNTapNone;
    if (onlyAnClick)
        btn.tapType = PageBTNTapAnimalNone;
    ...
    CGFloat btnWidth =
        [self getTitleData:titleInfo key:WMZPageKeyTitleWidth] ? [[self getTitleData:titleInfo key:WMZPageKeyTitleWidth] floatValue] : (self.param.wMenuTitleWidth ?: (size.width + margin));
    CGFloat marginX = 0;
    ...
    if (self.param.wMenuAnimal == PageTitleMenuCircleBg && self.param.wMenuTitleRadios == 0)
...
- (NSString *)getTitleData:(id)model key:(NSString *)key {
    if ([model isKindOfClass:[NSString class]] || [model isKindOfClass:[NSAttributedString class]])
        return [key isEqualToString:WMZPageKeyName] ? model : nil;
...
- (void)tap:(WMZPageNaviBtn *)btn {
    ...
    if (index == NSNotFound || index > self.btnArr.count)
        return;
...
- (void)scrollToIndex:(NSInteger)newIndex animal:(BOOL)animal {
    if (self.btnArr.count <= newIndex)
        return;
    if (animal && !self.lastBTN)
        animal = NO;
    WMZPageNaviBtn *btn = self.btnArr[newIndex];
    if (self.lastBTN) {
        self.lastBTN.selected = NO;
        self.lastBTN.titleLabel.font = self.param.wMenuTitleUIFont;
        if (self.param.wMenuTitleBackground)
            self.lastBTN.layer.backgroundColor = self.param.wMenuTitleBackground.CGColor;
        if (self.lastBTN.layer.backgroundColor && !self.param.wMenuTitleBackground)
            self.lastBTN.layer.backgroundColor = UIColor.clearColor.CGColor;
        if ([self getTitleData:self.lastBTN.config key:WMZPageKeyImage]) {
            [self.lastBTN tagSetImagePosition:self.param.wMenuImagePosition
                                      spacing:[self getTitleData:self.lastBTN.config key:WMZPageKeyImageOffset] ? [[self getTitleData:self.lastBTN.config key:WMZPageKeyImageOffset] floatValue] :
                                                                                                                  self.param.wMenuImageMargin];
        }
    ...
    if (backgroundColor) {
        ...
        if ([backgroundColor isKindOfClass:[NSArray class]]) {
            if ([backgroundColor count] > 1) {
                tempBackgroundColor = [UIColor bm_colorGradientChangeWithSize:CGSizeMake(self.contentSize.width, self.frame.size.height) direction:PageGradientChangeDirectionLevel
                                                                   startColor:backgroundColor[0]
                                                                     endColor:backgroundColor[1]];
    ...
    if (self.param.wMenuSelectTitleBackground)
        btn.layer.backgroundColor = self.param.wMenuSelectTitleBackground.CGColor;
    if ([self getTitleData:btn.config key:WMZPageKeyImage]) {
        [btn tagSetImagePosition:self.param.wMenuImagePosition
                         spacing:[self getTitleData:btn.config key:WMZPageKeyImageOffset] ? [[self getTitleData:btn.config key:WMZPageKeyImageOffset] floatValue] : self.param.wMenuImageMargin];
    }
    ...
    if ([self isScrollEnabled])
        [self setContentOffset:point animated:animal];
    ...
    if (self.param.wMenuAnimal == PageTitleMenuJD) {
        if (self.lastBTN != btn) {
            ...
            if (self.param.wEventCustomJDAnimal)
                self.param.wEventCustomJDAnimal(btn, btn.jdLayer);
            [btn jdAddLayer];
        }
    }
    self.lastBTN = btn;
...
- (void)animalAction:(UIScrollView *)scrollView lastContrnOffset:(CGFloat)lastContentOffset {
    ...
    if (self.param.wMenuAnimalTitleGradient) {
        ...
        UIColor *rightItemColor = [UIColor colorWithRed:tempBtn.unSelectedColorR + (1 - scale) * difR green:tempBtn.unSelectedColorG + (1 - scale) * difG
                                                   blue:tempBtn.unSelectedColorB + (1 - scale) * difB
                                                  alpha:rightA];
