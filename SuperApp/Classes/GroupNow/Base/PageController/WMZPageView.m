...
- (instancetype)initWithFrame:(CGRect)frame autoFix:(BOOL)autoFix param:(WMZPageParam *)param parentReponder:(UIResponder *)parentReponder {
    return [self initWithFrame:frame autoFix:autoFix source:NO param:param parentReponder:parentReponder];
}

- (instancetype)initWithFrame:(CGRect)frame autoFix:(BOOL)autoFix source:(BOOL)pageController param:(WMZPageParam *)param parentReponder:(UIResponder *)parentReponder {
...
- (void)setUpUI:(BOOL)clear {
    if (!UIEdgeInsetsEqualToEdgeInsets(self.originMenuInsets, self.param.wMenuInsets) && !UIEdgeInsetsEqualToEdgeInsets(self.originMenuInsets, UIEdgeInsetsMake(-1, -1, -1, -1))) {
    ...
    if ([self.parentResponder isKindOfClass:UIViewController.class] && self.autoFit) {
        ...
        if (parentVC.presentingViewController) {
            ...
            headY = (!self.param.wFromNavi && self.param.wMenuPosition != PageMenuPositionBottom) ? 0 : (!parentVC.navigationController.navigationBar.translucent ? 0 : PageVCNavBarHeight);
        }
        if (parentVC.parentViewController) {
            if ([parentVC.parentViewController isKindOfClass:[UINavigationController class]]) {
                ...
                headY = (!self.param.wFromNavi && self.param.wMenuPosition != PageMenuPositionBottom) ?
                            0 :
                ...
                if (parentVC.parentViewController.navigationController) {
                    headY = (!self.param.wFromNavi && self.param.wMenuPosition != PageMenuPositionBottom) ?
                                0 :
                                (!parentVC.parentViewController.navigationController.navigationBar.translucent ? 0 : PageVCNavBarHeight);
    ...
    if (self.param.wCustomFailGesture)
        self.downSc.wCustomFailGesture = self.param.wCustomFailGesture;
    if (self.param.wCustomSimultaneouslyGesture)
        self.downSc.wCustomFailGesture = self.param.wCustomSimultaneouslyGesture;
    ...
    if ([self.parentResponder isKindOfClass:WMZPageController.class]) {
        ...
        if (self.param.wDelegateWithResponser) {
            self.downSc.delegate = (id)self.parentResponder;
            ;
        } else {
            ...
            if ([parentVC.parentViewController isKindOfClass:WMZPageController.class]) {
                ...
                while ((pageVC = (WMZPageController *)[pageVC parentViewController])) {
                    if ([pageVC isKindOfClass:[WMZPageController class]]) {
                        if (pageVC.param.wTopSuspension && [pageVC.headViewSonView isKindOfClass:UIView.class] && pageVC.headViewSonView.frame.size.height)
                            break;
                    }
                }
                if (pageVC && pageVC != self.parentResponder)
                    scroll = NO;
            }
        }
    }
...
- (void)setUpMenuAndDataViewFrame {
    ...
    if ([self canTopSuspension]) {
        if ([self.parentResponder isKindOfClass:UIViewController.class] && self.autoFit) {
            ...
            if (!parentVC.parentViewController) {
                ...
                if (![parentVC.parentViewController isKindOfClass:[WMZPageController class]]) {
                    if (parentVC.navigationController && ![parentVC.navigationController isNavigationBarHidden]) {
    ...
    if (self.param.wMenuPosition == PageMenuPositionBottom) {
        ...
        if ([self.parentResponder isKindOfClass:UIViewController.class] && [(UIViewController *)self.parentResponder navigationController]) {
...
- (void)setUpHead {
    /// 头部视图
    if (self.param.wMenuHeadView && self.param.wMenuPosition != PageMenuPositionNavi && self.param.wMenuPosition != PageMenuPositionBottom) {
    ...
    if (self.head_MenuView) {
        self.head_MenuView.frame = CGRectMake(0,
                                              self.headView ? CGRectGetMinX(self.headView.frame) : CGRectGetMinX(self.upSc.frame),
                                              self.upSc.frame.size.width,
                                              CGRectGetMaxY(self.upSc.frame) - self.upSc.dataView.frame.size.height);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (yOffset <= 0) {
        ...
        if (yOffset >= (topOffset - 1) || yOffset >= (topOffset + 1) || yOffset >= topOffset)
    ...
    if (self.param.wAvoidQuickScroll && [self.currentScroll isKindOfClass:UIScrollView.class] && self.currentScroll.contentOffset.y && yOffset <= 0) {
        if (self.currentScroll.tag != 22222) {
            scrollView.contentOffset = CGPointMake(self.downSc.contentOffset.x, topOffset);
            self.scrolTotop = NO;
            self.canScroll = NO;
        } else {
            if (!self.currentScroll.isDragging) {
                scrollView.contentOffset = CGPointMake(self.downSc.contentOffset.x, topOffset);
                self.scrolTotop = NO;
                self.canScroll = NO;
            }
    ...
    //    HDLog(@"🐻 %f scrolTotop = %d scrolToBottom = %d canScroll = %d",yOffset,self.scrolTotop,self.scrolToBottom,self.sonCanScroll);
    if (self.scrolTotop) {
        self.sonCanScroll = YES;
        if ([self.currentScrollArr isKindOfClass:NSArray.class] && self.currentScrollArr.count > 1) {
            BOOL canScroll = NO;
            for (UIScrollView *sc in self.currentScrollArr) {
                if ([sc isKindOfClass:UIScrollView.class]) {
                    canScroll = (sc.contentSize.height <= sc.frame.size.height);
                    if (!canScroll)
                        break;
                }
            }
        ...
        if (!self.canScroll) {
            ...
            if ([self.currentScrollArr isKindOfClass:NSArray.class] && self.currentScrollArr.count > 1) {
                BOOL sonCanScroll = NO;
                UIScrollView *findSc = nil;
                for (UIScrollView *sc in self.currentScrollArr) {
                    if ([sc isKindOfClass:UIScrollView.class]) {
                        sonCanScroll = sc.contentOffset.y > 0;
                        if (sonCanScroll) {
                            findSc = sc;
                            break;
                        }
                ...
                self.sonCanScroll = sonCanScroll;
                if (sonCanScroll && yOffset < topOffset && self.canScroll && findSc.tag == 11111 && self.currentScroll == findSc) {
                    self.canScroll = NO;
                    [scrollView setContentOffset:CGPointMake(0, topOffset) animated:NO];
                }
            } else {
    ...
    if (self.param.wNaviAlpha) {
        if ([self.parentResponder isKindOfClass:UIViewController.class]) {
            if ([(UIViewController *)self.parentResponder navigationController] && self.naviBarBackGround)
                self.naviBarBackGround.alpha = delta;
        }
        if (self.param.wHeaderScrollHide && self.headView)
            self.headView.alpha = !(delta == 1);
    }
    ...
    /// 防止第一次加载不成功
    if (self.currentFootView && self.currentFootView.frame.origin.y != self.footOriginY)
...
- (void)changeMenuFrame {
    if (!self.param.wTopChangeHeight)
        return;
    if (self.upSc.mainView.frame.size.height == self.param.wMenuHeight && !self.sonCanScroll)
        return;
    CGFloat offsetHeight
        = self.param.wTopChangeHeight > 0 ? MIN(self.currentScroll.contentOffset.y, self.param.wTopChangeHeight) : MAX(-self.currentScroll.contentOffset.y, self.param.wTopChangeHeight);
    if (self.upSc.mainView.frame.size.height == (self.param.wMenuHeight + self.param.wMenuInsets.bottom - self.param.wTopChangeHeight) && self.sonCanScroll
        && offsetHeight == self.param.wTopChangeHeight)
        return;
...
- (void)setUpSuspension:(UIResponder *)newVC index:(NSInteger)index end:(BOOL)end {
    ...
    if ([newVC conformsToProtocol:@protocol(WMZPageProtocol)]) {
        ...
        if ([newVC respondsToSelector:@selector(fixFooterView)]) {
            ...
            if (self.currentFootView && end && !self.param.wFixFirst)
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *, id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"contentOffset"]) {
        ...
        if ([self.parentResponder isKindOfClass:UIViewController.class]) {
            ...
            while ((parentVC = (WMZPageController *)[parentVC parentViewController])) {
                if ([parentVC conformsToProtocol:@protocol(WMZPageScrollProcotol)]) {
                    if (parentVC.param.wTopSuspension && [parentVC.headViewSonView isKindOfClass:UIView.class] && parentVC.headViewSonView.frame.size.height) {
        ...
        if (!pageVC)
            pageVC = self;
        if (![pageVC canTopSuspension])
            return;
        if (![pageVC currentNotSuspennsion])
            return;
        if (hadWillDisappeal)
            return;
        UIScrollView *currentSc = (UIScrollView *)object;
        if (![currentSc isKindOfClass:UIScrollView.class])
            return;
        if (pageVC.currentScroll != currentSc)
            pageVC.currentScroll = currentSc;
        CGPoint newH = [[change objectForKey:@"new"] CGPointValue];
        CGPoint oldH = [[change objectForKey:@"old"] CGPointValue];
        //        HDLog(@"🐻%f %d %f %f",pageVC.currentScroll.frame.size.width,currentSc.isDragging, newH.y,oldH.y);
        if (newH.y == oldH.y)
            return;
        if (pageVC.param.wBounces)
            pageVC.scrolToBottom = NO;
        ...
        if (self.param.wSonScrollCanPull) {
            if ((int)newH.y <= 0) {
...
- (BOOL)exchangeMenuDataAtIndex:(NSInteger)index withMenuDataAtIndex:(NSInteger)replaceIndex {
    if (self.param.wTitleArr.count < index || self.param.wTitleArr.count < replaceIndex) {
    ...
    if (self.upSc.btnArr && self.upSc.btnArr.count > index && self.upSc.btnArr.count > replaceIndex) {
        [self.upSc.btnArr exchangeObjectAtIndex:index withObjectAtIndex:replaceIndex];
    }
    NSMutableArray *controllerArr = [NSMutableArray arrayWithArray:self.param.wControllers];
    if (controllerArr.count > index && controllerArr.count > replaceIndex) {
...
- (BOOL)canTopSuspension {
    if (!self.param.wTopSuspension || self.param.wMenuPosition == PageMenuPositionBottom || self.param.wMenuPosition == PageMenuPositionNavi)
        return NO;
...
- (BOOL)currentNotSuspennsion {
    if (self.param.wTitleArr.count > self.upSc.currentTitleIndex) {
        ...
        if ([data isKindOfClass:[NSDictionary class]] && data[WMZPageKeyCanTopSuspension] && ![data[WMZPageKeyCanTopSuspension] boolValue])
            return NO;
    }
    return YES;
}
