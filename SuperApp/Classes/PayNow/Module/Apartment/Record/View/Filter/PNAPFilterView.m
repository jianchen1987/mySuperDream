...
@interface PNAPFilterView () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([type isEqualToString:kBillType]) {
        ...
        ///
        if ([model.value isEqualToString:[NSString stringWithFormat:@"%zd", PNApartmentPaymentStatus_ALL]]) {
            for (PNBillFilterModel *model in sectionModel.list) {
                model.isSelected = NO;
            }
            model.isSelected = YES;
            [self.statusArray removeAllObjects];
            [self.statusArray addObject:@(PNApartmentPaymentStatus_ALL)];
        } else {
            ...
            if (model.isSelected) {
                ...
                [self.statusArray removeObject:@(PNApartmentPaymentStatus_ALL)];
            } else {
...
- (NSMutableArray<PNBillFilterModel *> *)billTypeDataSource {
    ...
    model.value = [NSString stringWithFormat:@"%zd", PNApartmentPaymentStatus_ALL];
    [arr addObject:model];

    model = PNBillFilterModel.new;
    model.titleName = [PNCommonUtils getApartmentStatusName:PNApartmentPaymentStatus_PAID];
    model.value = [NSString stringWithFormat:@"%zd", PNApartmentPaymentStatus_PAID];
    [arr addObject:model];

    model = PNBillFilterModel.new;
    model.titleName = [PNCommonUtils getApartmentStatusName:PNApartmentPaymentStatus_USER_HAS_UPLOADED_VOUCHER];
    model.value = [NSString stringWithFormat:@"%zd", PNApartmentPaymentStatus_USER_HAS_UPLOADED_VOUCHER];
    [arr addObject:model];

    model = PNBillFilterModel.new;
    model.titleName = [PNCommonUtils getApartmentStatusName:PNApartmentPaymentStatus_USER_REJECT];
    model.value = [NSString stringWithFormat:@"%zd", PNApartmentPaymentStatus_USER_REJECT];
    [arr addObject:model];

    model = PNBillFilterModel.new;
    model.titleName = [PNCommonUtils getApartmentStatusName:PNApartmentPaymentStatus_CLOSED];
    model.value = [NSString stringWithFormat:@"%zd", PNApartmentPaymentStatus_CLOSED];
    [arr addObject:model];
