...
static NSString *const kStatus = @"filter_transfer_status";
static NSString *const kStore = @"filter_store";
static NSString *const kOperator = @"filter_operator";


@interface PNMSFilterView () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
@property (nonatomic, strong) NSMutableArray *transferStatusArray;

@end


@implementation PNMSFilterView
...
- (void)resetAllStatus {
    for (HDTableViewSectionModel *sectionModel in self.dataSourceArray) {
        ...
        if ([type isEqualToString:kCurrencyType] || [type isEqualToString:kBillType] || [type isEqualToString:kStatus] || [type isEqualToString:kOperator] || [type isEqualToString:kStore]) {
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([type isEqualToString:kDateType]) {
        ...
        PNMSFilterCurrencyCell *cell = [PNMSFilterCurrencyCell cellWithCollectionView:collectionView forIndexPath:indexPath identifier:kBillType];
        PNMSBillFilterModel *model = [sectionModel.list objectAtIndex:indexPath.item];
        cell.model = model;
        return cell;
    } else if ([type isEqualToString:kCurrencyType]) {
        PNMSFilterCurrencyCell *cell = [PNMSFilterCurrencyCell cellWithCollectionView:collectionView forIndexPath:indexPath identifier:kCurrencyType];
        PNMSBillFilterModel *model = [sectionModel.list objectAtIndex:indexPath.item];
        cell.model = model;
        return cell;
    } else if ([type isEqualToString:kStatus]) {
        PNMSFilterCurrencyCell *cell = [PNMSFilterCurrencyCell cellWithCollectionView:collectionView forIndexPath:indexPath identifier:kStatus];
        PNMSBillFilterModel *model = [sectionModel.list objectAtIndex:indexPath.item];
        cell.model = model;
        return cell;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    HDTableViewSectionModel *sectionModel = [self.dataSourceArray objectAtIndex:indexPath.section];
    NSString *type = [sectionModel hd_getBoundObjectForKey:kFilterSectionType];
    if ([type isEqualToString:kCurrencyType] || [type isEqualToString:kBillType] || [type isEqualToString:kStatus] || [type isEqualToString:kOperator]) {
        for (PNMSBillFilterModel *model in sectionModel.list) {
            model.isSelected = NO;
        }
        PNMSBillFilterModel *model = [sectionModel.list objectAtIndex:indexPath.item];
        ...
        if ([type isEqualToString:kCurrencyType]) {
            self.filterModel.currency = model.value;
        }

        if ([type isEqualToString:kBillType]) {
            self.filterModel.transType = [model.value integerValue];
            if (self.filterModel.transType == PNTransTypeRecharge) {
                self.transferStatusArray = [self transferStatusDataSource];
            } else {
                [self.transferStatusArray removeAllObjects];
            }
            [self reloadData];
        }

        if ([type isEqualToString:kStatus]) {
            self.filterModel.transferStatus = [model.value integerValue];
        }
...
- (void)setDefaultCurrency:(NSMutableArray *)array {
    for (PNMSBillFilterModel *model in array) {
        if ([model.value isEqualToString:self.filterModel.currency]) {
            model.isSelected = YES;
            break;
        }
    }
...
- (NSMutableArray *)transferTypeDataSource {
    NSMutableArray *arr = [NSMutableArray array];
    PNMSBillFilterModel *model = PNMSBillFilterModel.new;
    model.titleName = PNLocalizedString(@"TRANS_TYPE_ALL", @"所有");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeDefault];
    [arr addObject:model];

    model = PNMSBillFilterModel.new;
    model.titleName = PNLocalizedString(@"TRANS_TYPE_COLLECT", @"收款");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeCollect];
    [arr addObject:model];

    model = PNMSBillFilterModel.new;
    model.titleName = PNLocalizedString(@"pn_Withdraw", @"提现");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeRecharge];
    [arr addObject:model];

    model = PNMSBillFilterModel.new;
    model.titleName = PNLocalizedString(@"TRANS_TYPE_REFUND", @"退款");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeRefund];
    [arr addObject:model];
...
- (void)setDefaultTransType:(NSMutableArray *)array {
    for (PNMSBillFilterModel *model in array) {
        if ([model.value integerValue] == self.filterModel.transType) {
            model.isSelected = YES;
            break;
        }
    }
...
- (NSMutableArray *)transferStatusDataSource {
    NSMutableArray *arr = [NSMutableArray array];

    PNMSBillFilterModel *model = PNMSBillFilterModel.new;
    model.titleName = PNLocalizedString(@"pn_all", @"全部");
    model.value = [NSString stringWithFormat:@"%zd", PNOrderStatusAll];
    [arr addObject:model];

    model = PNMSBillFilterModel.new;
    model.titleName = PNLocalizedString(@"ORDER_STATUS_SUCCESS", @"成功");
    model.value = [NSString stringWithFormat:@"%zd", PNOrderStatusSuccess];
    [arr addObject:model];

    model = PNMSBillFilterModel.new;
    model.titleName = PNLocalizedString(@"ORDER_STATUS_FAILURE", @"失败");
    model.value = [NSString stringWithFormat:@"%zd", PNOrderStatusFailure];
    [arr addObject:model];

    model = PNMSBillFilterModel.new;
    model.titleName = PNLocalizedString(@"ORDER_STATUS_PROCESS", @"处理中");
    model.value = [NSString stringWithFormat:@"%zd", PNOrderStatusProcessing];
    [arr addObject:model];

    [self setDefaultTransferStatsu:arr];

    return arr;
}

- (void)setDefaultTransferStatsu:(NSMutableArray *)array {
    for (PNMSBillFilterModel *model in array) {
        if ([model.value integerValue] == self.filterModel.transferStatus) {
            model.isSelected = YES;
            break;
        }
    }
...
- (void)initData {
    ...
    sectionModel.list = [self transferTypeDataSource];
    [self.dataSourceArray addObject:sectionModel];
    ...
    if (self.transferStatusArray.count > 0) {
        /// 交易状态
        sectionModel = HDTableViewSectionModel.new;
        [sectionModel hd_bindObjectWeakly:kStatus forKey:kFilterSectionType];
        headerModel = SACollectionReusableViewModel.new;
        headerModel.title = PNLocalizedString(@"5xdNPvU0", @"交易状态");
        headerModel.titleColor = HDAppTheme.PayNowColor.c343B4D;
        headerModel.titleFont = [HDAppTheme.PayNowFont fontMedium:15];
        sectionModel.commonHeaderModel = headerModel;

        sectionModel.list = self.transferStatusArray;
