...
@interface PNMSUploadVoucherView () <HDTextViewDelegate, HXPhotoViewDelegate>
...
@implementation PNMSUploadVoucherView
...
- (void)clickDoneAction {
    if (self.selectedPhotos.count > 0) {
        [self uploadImagesCompletion:^(NSArray<NSString *> *imgUrlArray) {
            ...
            [dict setValue:urlStr forKey:@"imgUrl"];
            [dict setValue:self.textView.text forKey:@"remark"];
            [self.viewModel saveVoucher:dict success:^(PNMSVoucherInfoModel *_Nonnull rspModel) {
                PNMSUploadVoucherResultViewController *vc = [[PNMSUploadVoucherResultViewController alloc] initWithRouteParameters:@{
                    @"id": rspModel.voucherId,
                }];
                NSMutableArray *newVCArr = [NSMutableArray arrayWithArray:self.viewController.navigationController.viewControllers];
                ...
                [self.viewController.navigationController setViewControllers:newVCArr animated:YES];
            }];
        }];
    }
}
...
- (SALabel *)titleLabel {
    if (!_titleLabel) {
        SALabel *label = SALabel.new;
        NSString *hightStr = @"*";
        NSString *allStr = [NSString stringWithFormat:@"%@%@", hightStr, PNLocalizedString(@"pn_Voucher", @"支付凭证")];
