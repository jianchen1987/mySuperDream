...
@implementation PNNorPacketView
...
- (PNPacketSwitchHandOutItemView *)switchItemView {
    if (!_switchItemView) {
        ...
        _switchItemView.packetType = PNPacketType_Nor;
    }
...
- (PNPacketRateTipsItemView *)tipsItemView {
    if (!_tipsItemView) {
        _tipsItemView = [[PNPacketRateTipsItemView alloc] init];
    }
...
- (PNOperationButton *)sendBtn {
    if (!_sendBtn) {
        ...
        _sendBtn.enabled = NO;
        [_sendBtn setTitle:PNLocalizedString(@"pn_prepre_red_packet", @"塞进红包") forState:UIControlStateNormal];
