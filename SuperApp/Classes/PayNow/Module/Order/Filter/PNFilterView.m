...
@interface PNFilterView () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([type isEqualToString:kCurrencyType] || [type isEqualToString:kBillType]) {
        ...
        if ([type isEqualToString:kBillType]) {
            self.transType = [model.value integerValue];
        }
        [self.collectionView successGetNewDataWithNoMoreData:NO];
    }
}
...
- (NSMutableArray<PNBillFilterModel *> *)billTypeDataSource {
    ...
    model.titleName = PNLocalizedString(@"consumption", @"消费");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeConsume];
    [arr addObject:model];

    model = PNBillFilterModel.new;
    model.titleName = PNLocalizedString(@"TRANS_TYPE_TRANSFER", @"转账");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeTransfer];
    [arr addObject:model];
    ...
    model = PNBillFilterModel.new;
    model.titleName = PNLocalizedString(@"Deposit", @"入金");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeRecharge];
    [arr addObject:model];

    model = PNBillFilterModel.new;
    model.titleName = PNLocalizedString(@"Cash_out", @"出金");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeWithdraw];
    [arr addObject:model];
    ...
    model = PNBillFilterModel.new;
    model.titleName = PNLocalizedString(@"TRANS_TYPE_Remuneration", @"酬劳");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeRemuneration];
    [arr addObject:model];

    model = PNBillFilterModel.new;
    model.titleName = PNLocalizedString(@"redpacket", @"红包");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypePinkPacket];
    [arr addObject:model];

    model = PNBillFilterModel.new;
    model.titleName = PNLocalizedString(@"pn_bill", @"缴费");
    model.value = [NSString stringWithFormat:@"%zd", PNTransTypeApartment];
    [arr addObject:model];
...
- (void)setDefaultTransType:(NSMutableArray *)array {
    for (PNBillFilterModel *model in array) {
        if ([model.value integerValue] == self.transType) {
            model.isSelected = YES;
            break;
        }
    }
...
- (void)initData {
    ...
    headerModel.titleFont = [HDAppTheme.PayNowFont fontMedium:15];
    sectionModel.commonHeaderModel = headerModel;
    if (self.delegate && [self.delegate respondsToSelector:@selector(tradeTypeDataSource)]) {
        NSMutableArray *listArr = [NSMutableArray arrayWithArray:[self.delegate tradeTypeDataSource]];
        sectionModel.list = listArr;
        [self setDefaultTransType:listArr];
    } else {
