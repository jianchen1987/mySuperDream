...
@property (nonatomic, strong) HDCustomNavigationBar *navigationBar;            ///< 导航栏
...
- (void)addOrUpdateWrapperViewFrame {
    CGRect oldFrame = self.viewWrapper.frame;

    if (self.isNavBarTransparent) {
        self.viewWrapper.frame = CGRectMake(0, 0, CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame));
    } else {
        if (_navigationBar && !self.navigationBar.isHidden) {
            self.viewWrapper.frame
                = CGRectMake(0, CGRectGetMaxY(self.navigationBar.frame), CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame) - CGRectGetHeight(self.navigationBar.frame));
        } else {
            self.viewWrapper.frame = CGRectMake(0, 0, CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame));
        }
    }
    CGRect newFrame = self.viewWrapper.frame;
...
- (void)setHideViewWrapper:(BOOL)hideViewWrapper {
    ...
    self.viewWrapper.hidden = hideViewWrapper;
}

#pragma mark - lazy load
- (HDCustomNavigationBar *)navigationBar {
    if (!_navigationBar) {
        _navigationBar = [HDCustomNavigationBar customNavigationBar];
