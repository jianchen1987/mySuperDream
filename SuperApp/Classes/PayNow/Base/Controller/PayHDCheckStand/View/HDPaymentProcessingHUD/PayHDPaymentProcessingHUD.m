...
#define ThemeColor [UIColor hd_colorWithHexString:@"#FD7127"]

typedef void (^CompletionHandler)(void);


@interface PayHDPaymentProcessingHUD () <CAAnimationDelegate>
...
@implementation PayHDPaymentProcessingHUD
...
+ (PayHDPaymentProcessingHUD *)showLoadingIn:(UIView *)view offset:(CGFloat)offset {
    [self hideFor:view];
    PayHDPaymentProcessingHUD *hud = [[PayHDPaymentProcessingHUD alloc] initWithFrame:view.bounds];
    hud.offset = offset;
    [hud startRotating];
    [view addSubview:hud];
    return hud;
...
+ (PayHDPaymentProcessingHUD *)hideFor:(UIView *)view {
    PayHDPaymentProcessingHUD *hud;
    for (PayHDPaymentProcessingHUD *subView in view.subviews) {
        if ([subView isKindOfClass:PayHDPaymentProcessingHUD.class]) {
            [subView hide];
            hud = subView;
        }
    }
