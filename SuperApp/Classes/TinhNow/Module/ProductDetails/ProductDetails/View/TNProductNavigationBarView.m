...
@interface TNProductNavigationBarView () <HDCategoryViewDelegate>
/// 背景
...
@implementation TNProductNavigationBarView
...
- (void)updateNavBarRightImages:(CGFloat)rate {
    if (rate > 0.6) {
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [self.cartBTN setImage:[UIImage imageNamed:@"tn_product_navbar_shopcar"] forState:UIControlStateNormal];
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [self.cartBTN setImage:[UIImage imageNamed:@"tn_product_navbar_shopcar_lucency"] forState:UIControlStateNormal];
...
- (TNShoppingCartButton *)cartBTN {
    if (!_cartBTN) {
        ...
        [_cartBTN setImage:[UIImage imageNamed:@"tn_product_navbar_shopcar_lucency"] forState:UIControlStateNormal];
        _cartBTN.imageEdgeInsets = UIEdgeInsetsMake(10, 0, 10, 10);
        ...
        [_cartBTN addTouchUpInsideHandler:^(UIButton *_Nonnull btn) {
            @HDStrongify(self);
            !self.shopCartcallBack ?: self.shopCartcallBack();
        }];
    }
    return _cartBTN;
}
...
- (HDCategoryTitleView *)sectionTitleView {
    if (!_sectionTitleView) {
        ...
        lineView.indicatorColor = [UIColor tn_colorGradientChangeWithSize:CGSizeMake(kRealWidth(20), kRealWidth(3)) direction:TNGradientChangeDirectionLevel startColor:HexColor(0xFF8818)
                                                                 endColor:[HexColor(0xFFBD35) colorWithAlphaComponent:0.1]];
        //        HDAppTheme.TinhNowColor.C1;
        lineView.indicatorWidth = kRealWidth(20);
        lineView.indicatorHeight = kRealWidth(3);
        lineView.verticalMargin = kRealWidth(5);
