...
@interface TNSKUDataFilter () {
    TNSKUCondition *_defaultSku;
}

@property (nonatomic, strong) NSSet<TNSKUCondition *> *conditions;
...
@implementation TNSKUDataFilter
...
- (void)initPropertiesSkuListData {
    ...
    for (int i = 0; i < [_dataSource numberOfConditionsInFilter:self]; i++) {
        TNSKUCondition *model = [TNSKUCondition new];
    ...
    if (_defaultSku) {
        [_defaultSku.properties enumerateObjectsUsingBlock:^(TNSKUProperty *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
//获取属性
- (NSArray<TNSKUProperty *> *)propertiesWithConditionRawData:(NSArray *)data {
...
- (TNSKUProperty *)propertyOfValue:(id)value inSection:(NSInteger)section {
    ...
    return [[TNSKUProperty alloc] initWithValue:value indexPath:indexPath];
}
...
- (NSMutableSet<NSIndexPath *> *)getAllAvailableIndexPaths {
    ...
    [_conditions enumerateObjectsUsingBlock:^(TNSKUCondition *_Nonnull obj, BOOL *_Nonnull stop) {
...
- (NSMutableSet<NSIndexPath *> *)availableIndexPathsFromSelctedIndexPath:(NSIndexPath *)selectedIndexPath sectedIndexPaths:(NSSet<NSIndexPath *> *)indexPaths {
    ...
    [_conditions enumerateObjectsUsingBlock:^(TNSKUCondition *_Nonnull obj, BOOL *_Nonnull stop) {
        if ([obj.conditionIndexs objectAtIndex:selectedIndexPath.section].integerValue == selectedIndexPath.item) {
            [obj.properties enumerateObjectsUsingBlock:^(TNSKUProperty *_Nonnull property, NSUInteger idx2, BOOL *_Nonnull stop1) {
...
#pragma mark-- setter
- (void)setDataSource:(id<TNSKUDataFilterDataSource>)dataSource {
    _dataSource = dataSource;
    [self reloadData];
}
...
@implementation TNSKUCondition

- (void)setProperties:(NSArray<TNSKUProperty *> *)properties {
    ...
    [properties enumerateObjectsUsingBlock:^(TNSKUProperty *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
