...
+ (MixpanelIdentity *)loadIdentity:(NSString *)apiToken {
    ...
    return [[MixpanelIdentity alloc] initWithDistinctId:distinctId peopleDistinctId:peopleDistinctId anonymousId:anonymousId userId:userId alias:alias hadPersistedDistinctId:hadPersistedDistinctId];
}
...
- (void)migrate {
    ...
    MixpanelIdentity *mixpanelIdentity = [[MixpanelIdentity alloc] initWithDistinctId:properties[kLegacyDefaultKeyDistinctId] peopleDistinctId:properties[kLegacyDefaultKeyPeopleDistinctId]
...
+ (id)unarchiveFromFile:(NSString *)filePath asClass:(Class)class {
    ...
    @try {
        if (@available(iOS 11, macOS 10.13, tvOS 11, watchOS 4, *)) {
            ...
            unarchivedData = [NSKeyedUnarchiver
                unarchivedObjectOfClasses:[NSSet
                                              setWithArray:@[[NSArray class], [NSDictionary class], [NSSet class], [NSString class], [NSDate class], [NSURL class], [NSNumber class], [NSNull class]]]
                                 fromData:data
                                    error:&error];
            if (error) {
...
- (BOOL)needMigration {
    ...
    return [manager fileExistsAtPath:[self eventsFilePath]] || [manager fileExistsAtPath:[self peopleFilePath]] || [manager fileExistsAtPath:[self groupsFilePath]] ||
           [manager fileExistsAtPath:[self propertiesFilePath]] || [manager fileExistsAtPath:[self optOutFilePath]];
}
@end
