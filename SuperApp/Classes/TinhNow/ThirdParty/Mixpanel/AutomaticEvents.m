...
- (void)initializeEvents:(MixpanelPeople *)peopleInstance apiToken:(NSString *)apiToken {
    ...
    if (defaults != nil && infoDict != nil) {
        ...
        if (appVersionValue != nil && savedVersionValue != nil && [appVersionValue compare:savedVersionValue options:NSNumericSearch] == NSOrderedDescending) {
...
- (void)appWillResignActive:(NSNotification *)notification {
    ...
    if (sessionLength >= (double)(self.minimumSessionDuration / 1000) && sessionLength <= (double)(self.maximumSessionDuration / 1000)) {
        NSMutableDictionary *properties = [[NSMutableDictionary alloc] initWithObjectsAndKeys:[NSNumber numberWithDouble:sessionLength], @"$ae_session_length", nil];
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response {
    @synchronized(awaitingTransactions) {
        for (SKProduct *product in response.products) {
            ...
            if (transaction != nil) {
                [self.delegate track:@"$ae_iap"
                          properties:@{@"$ae_iap_price": product.price, @"$ae_iap_quantity": [NSNumber numberWithInteger:transaction.payment.quantity], @"$ae_iap_name": product.productIdentifier}];
            }
        }
    }
}
