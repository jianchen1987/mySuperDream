...
+ (Mixpanel *)sharedInstanceWithToken:(NSString *)apiToken
    ...
    return [[self alloc] initWithToken:apiToken trackAutomaticEvents:trackAutomaticEvents flushInterval:flushInterval trackCrashes:trackCrashes optOutTrackingByDefault:optOutTrackingByDefault
...
- (instancetype)initWithToken:(NSString *)apiToken
    ...
    if (self = [self init:apiToken]) {
        ...
        self.useIPAddressForGeoLocation = NO;
        self.shouldManageNetworkActivityIndicator = YES;
        self.flushOnBackground = YES;

        self.serverURL = SAAppEnvManager.sharedInstance.appEnvConfig.tinhNowLog;
        //        @"http://172.16.20.11:6443";
...
- (instancetype)initWithToken:(NSString *)apiToken
         trackAutomaticEvents:(BOOL)trackAutomaticEvents
                flushInterval:(NSUInteger)flushInterval
                 trackCrashes:(BOOL)trackCrashes
          useUniqueDistinctId:(BOOL)useUniqueDistinctId {
    return [self initWithToken:apiToken trackAutomaticEvents:trackAutomaticEvents flushInterval:flushInterval trackCrashes:trackCrashes optOutTrackingByDefault:NO
            useUniqueDistinctId:useUniqueDistinctId];
}
...
- (instancetype)initWithToken:(NSString *)apiToken trackAutomaticEvents:(BOOL)trackAutomaticEvents andFlushInterval:(NSUInteger)flushInterval {
    return [self initWithToken:apiToken trackAutomaticEvents:trackAutomaticEvents flushInterval:flushInterval trackCrashes:NO useUniqueDistinctId:NO];
}
...
- (void)checkIfImplemented:(NSString *)distinctId debugInitCount:(NSInteger)debugInitCount {
    ...
    if (!hasImplemented) {
        ...
        if (completed >= 3) {
            [self sendHttpEvent:@"SDK Implemented" apiToken:@"metrics-1" distinctId:distinctId
                     properties:@{@"Tracked": @(hasTracked), @"Identified": @(hasIdentified), @"Aliased": @(hasAliased), @"Used People": @(hasUsedPeople), @"Debug Launch Count": @(debugInitCount)}];
...
+ (void)assertPropertyType:(id)propertyValue depth:(NSUInteger)depth {
    ...
    NSAssert([propertyValue isKindOfClass:[NSString class]] || [propertyValue isKindOfClass:[NSNumber class]] || [propertyValue isKindOfClass:[NSNull class]] ||
                 [propertyValue isKindOfClass:[NSArray class]] || [propertyValue isKindOfClass:[NSDictionary class]] || [propertyValue isKindOfClass:[NSDate class]] ||
                 [propertyValue isKindOfClass:[NSURL class]],
             @"%@ property values must be NSString, NSNumber, NSNull, NSArray, NSDictionary, NSDate or NSURL. got: %@ %@",
             self,
             [propertyValue class],
             propertyValue);
...
- (MixpanelIdentity *)currentMixpanelIdentity {
    return [[MixpanelIdentity alloc] initWithDistinctId:self.distinctId peopleDistinctId:self.people.distinctId anonymousId:self.anonymousId userId:self.userId alias:self.alias
...
- (void)track:(NSString *)event properties:(NSDictionary *)properties {
    ...
    dispatch_async(self.serialQueue, ^{
        ...
        NSMutableDictionary *e = [[NSMutableDictionary alloc] initWithDictionary:@{@"event": event, @"properties": [NSDictionary dictionaryWithDictionary:p]}];
...
- (void)addValuesToGroupSuperProperty:(NSString *)groupKey groupID:(id<MixpanelType>)groupID {
    [self updateSuperPropertiesAsync:^NSDictionary *(NSDictionary *superProps) {
        ...
        NSMutableArray<id<MixpanelType>> *values = [NSMutableArray arrayWithArray:mutableSuperProps[groupKey]];
        BOOL exist = NO;
...
- (void)removeGroup:(NSString *)groupKey groupID:(id<MixpanelType>)groupID {
    ...
    [self updateSuperPropertiesAsync:^NSDictionary *(NSDictionary *superProps) {
        ...
        NSMutableArray *vals = [NSMutableArray arrayWithArray:mutableSuperProps[groupKey]];
...
- (MixpanelGroup *)getGroup:(NSString *)groupKey groupID:(id<MixpanelType>)groupID {
    ...
    @synchronized(self.cachedGroups) {
        ...
        if (!group || group.groupKey != groupKey || ![groupID equalToMixpanelType:group.groupID]) {
...
- (void)startFlushTimer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.flushInterval > 0) {
            self.timer = [NSTimer scheduledTimerWithTimeInterval:self.flushInterval target:self selector:@selector(flush) userInfo:nil repeats:YES];
...
- (void)setUpListeners {
    ...
    // Application lifecycle events
    [notificationCenter addObserver:self selector:@selector(applicationWillResignActive:) name:NSApplicationWillResignActiveNotification object:nil];
...
- (void)applicationDidEnterBackground:(NSNotification *)notification {
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:trackedKey]) {
        ...
        [self sendHttpEvent:@"Integration" apiToken:@"85053bf24bba75239b16a601d9387e17" distinctId:self.apiToken properties:@{} updatePeople:NO
...
- (void)sendHttpEvent:(NSString *)eventName apiToken:(NSString *)apiToken distinctId:(NSString *)distinctId {
...
- (void)sendHttpEvent:(NSString *)eventName apiToken:(NSString *)apiToken distinctId:(NSString *)distinctId properties:(NSDictionary *)properties {
...
- (void)sendHttpEvent:(NSString *)eventName
    ...
    NSMutableDictionary *trackingProperties = [[NSMutableDictionary alloc] initWithDictionary:@{
        @"token": apiToken,
        @"mp_lib": @"iphone",
        @"distinct_id": distinctId,
        @"$lib_version": self.libVersion,
        @"Logging Enabled": @(self.enableLogging),
