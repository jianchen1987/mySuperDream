...
- (void)flushQueue:(NSArray *)queue endpoint:(MPNetworkEndpoint)endpoint persistenceType:(NSString *)persistenceType {
    ...
    while (queueCopyForFlushing.count > 0) {
        ...
        //        HDLog(@"%@ flushing %lu of %lu to %lu: %@", self, (unsigned long)batch.count, (unsigned long)queueCopyForFlushing.count, endpoint, queueCopyForFlushing);
        ...
        [[[MPNetwork sharedURLSession] dataTaskWithRequest:request completionHandler:^(NSData *responseData, NSURLResponse *urlResponse, NSError *error) {
...
- (BOOL)handleNetworkResponse:(NSHTTPURLResponse *)response withError:(NSError *)error {
    ...
    if (failed) {
        //        HDLog(@"电商埋点失败 statusCode = %ld", response.statusCode);
...
+ (NSString *)pathForEndpoint:(MPNetworkEndpoint)endpoint {
    ...
    dispatch_once(&onceToken, ^{
        endPointToPath = @{@(MPNetworkEndpointTrack): @"/static/images/2.gif", @(MPNetworkEndpointEngage): @"/static/images/1.gif", @(MPNetworkEndpointGroups): @"/static/images/3.gif"};
    });
    NSNumber *key = @(endpoint);
    return endPointToPath[key];
}

- (NSURLRequest *)buildGetRequestForEndpoint:(MPNetworkEndpoint)endpoint withQueryItems:(NSArray<NSURLQueryItem *> *)queryItems {
    return [self buildRequestForEndpoint:[MPNetwork pathForEndpoint:endpoint] byHTTPMethod:@"GET" withQueryItems:queryItems andBody:nil];
}

- (NSURLRequest *)buildPostRequestForEndpoint:(MPNetworkEndpoint)endpoint withQueryItems:(NSArray<NSURLQueryItem *> *)queryItems andBody:(NSString *)body {
    return [self buildRequestForEndpoint:[MPNetwork pathForEndpoint:endpoint] byHTTPMethod:@"POST" withQueryItems:queryItems andBody:body];
}

- (NSURLRequest *)buildRequestForEndpoint:(NSString *)endpoint byHTTPMethod:(NSString *)method withQueryItems:(NSArray<NSURLQueryItem *> *)queryItems andBody:(NSString *)body {
    // Build URL from path and query items
    NSURL *urlWithEndpoint = [self.serverURL URLByAppendingPathComponent:endpoint];
    NSURLComponents *components = [NSURLComponents componentsWithURL:urlWithEndpoint resolvingAgainstBaseURL:YES];
    if (queryItems) {
    ...
    [request setValue:@"tinhnow" forHTTPHeaderField:@"Content-Encoding"];
    [request setHTTPMethod:method];
    NSData *data = [body dataUsingEncoding:NSUTF8StringEncoding];
    NSData *gzipData = [data deflatedData];
