...
- (void)layoutSubviews {
    ...
    if (!CGRectEqualToRect(_previousBounds, bounds)) {
        ...
        if (!_scrollEnabled)
            _leftLabel.sj_w = bounds.size.width;
        [self _reset];
    }
}
...
- (void)_setRightFadeMask {
    [CATransaction begin];
    [CATransaction setDisableActions:YES];
    _fadeMaskLayer.colors = @[(__bridge id)[UIColor colorWithWhite:1 alpha:1.0].CGColor, (__bridge id)[UIColor colorWithWhite:1 alpha:0.05].CGColor];
...
- (void)_removeFadeMasks {
    [CATransaction begin];
    [CATransaction setDisableActions:YES];
    _fadeMaskLayer.colors = @[(__bridge id)[UIColor colorWithWhite:1 alpha:1.0].CGColor, (__bridge id)[UIColor colorWithWhite:1 alpha:1.0].CGColor];
