...
- (void)sjv_disapear {
    ...
    switch (self.sjv_disappearDirection) {
        case SJViewDisappearAnimation_None:
            break;
...
void __attribute__((overloadable)) sj_view_makeAppear(UIView *view, BOOL animated, void (^_Nullable completionHandler)(void)) {
    if (!view)
        return;
    sj_view_makeAppear(@[view], animated, completionHandler);
}
void __attribute__((overloadable)) sj_view_makeDisappear(UIView *view, BOOL animated, void (^_Nullable completionHandler)(void)) {
    if (!view)
        return;
...
void __attribute__((overloadable)) sj_view_makeAppear(NSArray<UIView *> *views, BOOL animated) {
    sj_view_makeAppear(views, animated, nil);
}
void sj_view_makeAppear(NSArray<UIView *> *views, BOOL animated, void (^_Nullable completionHandler)(void)) {
    if (views.count == 0)
        return;
    for (UIView *view in views) {
        ...
        [UIView animateWithDuration:0 animations:^{
            ...
            if (animated) {
                [UIView animateWithDuration:SJVideoPlayerConfigurations.shared.animationDuration animations:^{
                    ...
                    if (view == views.lastObject && completionHandler)
                        completionHandler();
                }];
            } else {
                [view sjv_appear];
...
void __attribute__((overloadable)) sj_view_makeDisappear(NSArray<UIView *> *views, BOOL animated) {
    sj_view_makeDisappear(views, animated, nil);
}
void sj_view_makeDisappear(NSArray<UIView *> *views, BOOL animated, void (^_Nullable completionHandler)(void)) {
    if (views.count == 0)
        return;
    for (UIView *view in views) {
