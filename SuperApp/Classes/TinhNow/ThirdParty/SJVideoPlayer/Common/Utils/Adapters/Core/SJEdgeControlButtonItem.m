...
- (instancetype)initWithTitle:(nullable NSAttributedString *)title target:(nullable id)target action:(nullable SEL)action tag:(SJEdgeControlButtonItemTag)tag {
...
- (instancetype)initWithImage:(nullable UIImage *)image target:(nullable id)target action:(nullable SEL)action tag:(SJEdgeControlButtonItemTag)tag {
    self = [self initWithTag:tag];
    if (!self)
        return nil;
    _image = image;
...
- (instancetype)initWithCustomView:(nullable __kindof UIView *)customView tag:(SJEdgeControlButtonItemTag)tag {
    self = [self initWithTag:tag];
    if (!self)
        return nil;
...
+ (instancetype)placeholderWithType:(SJButtonItemPlaceholderType)placeholderType tag:(SJEdgeControlButtonItemTag)tag {
    ...
    if (placeholderType == SJButtonItemPlaceholderType_49xFill)
        item.fill = YES;
    return item;
}
