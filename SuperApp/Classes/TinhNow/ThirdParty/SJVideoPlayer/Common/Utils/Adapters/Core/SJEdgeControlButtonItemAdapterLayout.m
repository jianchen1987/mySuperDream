...
- (void)_prepareLayout_Horizontal {
    ...
    _intrinsicContentSize = CGSizeMake(CGRectGetMaxX(_layoutAttributes.lastObject.frame), CGRectGetMaxY(_layoutAttributes.lastObject.frame));
}
...
- (CGPoint)center {
    return (CGPoint){_frame.origin.x + self.size.width * 0.5, _frame.origin.y + self.size.height * 0.5};
}
@end
