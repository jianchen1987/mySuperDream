...
- (void)setImage:(nullable UIImage *)image {
    ...
    if (_setImageExeBlock)
        _setImageExeBlock(self);
}
@end
...
- (void)handleTapGR:(UITapGestureRecognizer *)tap {
    if (_containerView.frame.size.width == 0)
        return;
...
- (void)setThumbCornerRadius:(CGFloat)thumbCornerRadius size:(CGSize)size thumbBackgroundColor:(UIColor *)thumbBackgroundColor {
...
- (void)setValue:(CGFloat)value_new animated:(BOOL)animated {
    ...
    if (value_new < _minValue)
        value_new = _minValue;
    else if (value_new > _maxValue)
        value_new = _maxValue;
    //    if ( _showsStopNode ) {
    ...
    if (animated) {
        CGFloat duration = 0;
        if (animated)
            duration = [self _calculateAnimaDuration:value_new - value_old];
...
- (UIActivityIndicatorView *)indicatorView {
    ...
    [_thumbImageView addConstraint:[NSLayoutConstraint constraintWithItem:_indicatorView attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:_thumbImageView
                                                                attribute:NSLayoutAttributeCenterX
                                                               multiplier:1
                                                                 constant:0]];
    [_thumbImageView addConstraint:[NSLayoutConstraint constraintWithItem:_indicatorView attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:_thumbImageView
...
- (void)_needUpdateThumbLayout {
    ...
    if (tracePosition <= minCenterX)
        tracePosition = minCenterX;
    else if (tracePosition >= maxCenterX)
        tracePosition = maxCenterX;
...
- (void)setShowsBufferProgress:(BOOL)showsBufferProgress {
    if (showsBufferProgress == _showsBufferProgress)
        return;
...
- (UIColor *)bufferProgressColor {
    if (_bufferProgressColor)
        return _bufferProgressColor;
...
- (void)_needUpdateBufferViewLayout {
    ...
    if (_showsStopNode && progress > _stopNodeLocation)
        progress = _stopNodeLocation;
...
- (UILabel *)promptLabel {
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:_promptLabel attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:_traceImageView
                                                     attribute:NSLayoutAttributeTrailing
                                                    multiplier:1
                                                      constant:0]];
    [self addConstraint:_promptLabelBottomConstraint = [NSLayoutConstraint constraintWithItem:_promptLabel attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:_thumbImageView
                                                                                    attribute:NSLayoutAttributeTop
                                                                                   multiplier:1
                                                                                     constant:-_promptSpacing]];
    return _promptLabel;
}
...
- (void)setStopNodeLocation:(CGFloat)stopNodeLocation {
    if (stopNodeLocation > 1)
        stopNodeLocation = 1;
    else if (stopNodeLocation < 0)
        stopNodeLocation = 0;
