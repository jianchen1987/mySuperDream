...
- (BOOL)videoPlayer:(__kindof SJBaseVideoPlayer *)videoPlayer gestureRecognizerShouldTrigger:(SJPlayerGestureType)type location:(CGPoint)location {
    ...
    if (!CGRectContainsPoint(adapter.view.frame, point))
        return YES;
...
- (void)videoPlayer:(SJBaseVideoPlayer *)videoPlayer reachabilityChanged:(SJNetworkStatus)status {
    ...
    if (_disabledPromptWhenNetworkStatusChanges)
        return;
    if ([self.videoPlayer.assetURL isFileURL])
        return; // return when is local video.
...
- (SJTimerControl *)lockStateTappedTimerControl {
    if (_lockStateTappedTimerControl)
...
- (BOOL (^)(SJEdgeControlLayer *_Nonnull))shouldShowCustomStatusBar {
    if (_shouldShowCustomStatusBar == nil) {
        ...
        [self setShouldShowCustomStatusBar:^BOOL(SJEdgeControlLayer *_Nonnull controlLayer) {
            if (controlLayer.videoPlayer.isFitOnScreen)
                return NO;
...
- (void)_addItemsToBottomAdapter {
    ...
    // 时间分隔符
    SJEdgeControlButtonItem *separatorItem =
        [[SJEdgeControlButtonItem alloc] initWithTitle:[NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol> _Nonnull make) {
...
- (void)_updateAppearStateForBottomProgressIndicatorIfNeeded {
    ...
    _videoPlayer.isControlLayerAppeared && !_videoPlayer.isLockedScreen ? sj_view_makeDisappear(_bottomProgressIndicator, YES) : sj_view_makeAppear(_bottomProgressIndicator, YES);
}
...
- (void)_reloadSizeForBottomTimeLabel {
    ...
    if (!durationTimeItem && !currentTimeItem)
        return;
...
- (void)_updateContentForBottomProgressSliderItemIfNeeded {
    if (!sj_view_isDisappeared(_bottomContainerView)) {
        ...
        if (!slider.isDragging)
            slider.value = _videoPlayer.currentTime;
...
- (void)_updateForDraggingProgressPopupView {
    ...
    if (!_videoPlayer.URLAsset.isM3u8 && [_videoPlayer.playbackController respondsToSelector:@selector(screenshotWithTime:size:completion:)]) {
...
- (void)_resetControlLayerAppearIntervalForItemIfNeeded:(NSNotification *)note {
    ...
    if (item.resetsAppearIntervalWhenPerformingItemAction) {
        if ([_topAdapter containsItem:item] || [_leftAdapter containsItem:item] || [_bottomAdapter containsItem:item] || [_rightAdapter containsItem:item])
...
- (void)_didMove:(NSTimeInterval)progressTime {
    ...
    if (_draggingProgressPopupView.isPreviewImageHidden == NO) {
        __weak typeof(self) _self = self;
        [_videoPlayer screenshotWithTime:progressTime size:CGSizeMake(_draggingProgressPopupView.frame.size.width, _draggingProgressPopupView.frame.size.height)
