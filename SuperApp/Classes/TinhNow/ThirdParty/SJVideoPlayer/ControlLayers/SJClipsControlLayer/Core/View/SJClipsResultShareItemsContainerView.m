//
//  SJClipsResultShareItemsContainerView.m
//  SJVideoPlayer
//
...
@implementation SJClipsResultShareItemsContainerView

- (void)clickedBtn:(UIButton *)btn {
    if (_clickedShareItemExeBlock)
        _clickedShareItemExeBlock(self, _shareItems[btn.tag]);
}
...
- (void)setShareItems:(nullable NSArray<SJClipsResultShareItem *> *)shareItems {
    ...
    [shareItems enumerateObjectsUsingBlock:^(SJClipsResultShareItem *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.titleLabel.numberOfLines = 0;
        [btn setAttributedTitle:[NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol> _Nonnull make) {
                 make.lineSpacing(8).alignment(NSTextAlignmentCenter).font([UIFont systemFontOfSize:10]).textColor([UIColor whiteColor]);
