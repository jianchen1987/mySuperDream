            ...
            switch (status) {
                ...
                case PHAuthorizationStatusDenied: {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (self.completionHandler)
                            self.completionHandler(NO, [[SJClipsSaveResultFailed alloc] initWithReason:SJClipsSaveResultToAlbumFailedReasonAuthDenied]);
                    });
                } break;
                ...
                case PHAuthorizationStatusAuthorized: {
                    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            if (self.completionHandler)
                                self.completionHandler(!error, error ? [[SJClipsSaveResultFailed alloc] initWithReason:SJClipsSaveResultToAlbumFailedReasonAuthDenied] : nil);
                        });
                    }];
                } break;
            }
...
- (void)video:(NSString *)videoPath didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (_completionHandler)
        _completionHandler(!error, error ? [[SJClipsSaveResultFailed alloc] initWithReason:SJClipsSaveResultToAlbumFailedReasonAuthDenied] : nil);
}
