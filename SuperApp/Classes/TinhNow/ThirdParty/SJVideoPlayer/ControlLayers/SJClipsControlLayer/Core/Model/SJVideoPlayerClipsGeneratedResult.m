...
- (void)setExportState:(SJClipsExportState)exportState {
    ...
    if (_exportStateDidChangeExeBlock)
        _exportStateDidChangeExeBlock(self);
}

- (void)setExportProgress:(float)exportProgress {
    _exportProgress = exportProgress;
    if (_exportProgressDidChangeExeBlock)
        _exportProgressDidChangeExeBlock(self);
}
...
- (void)setUploadState:(SJClipsResultUploadState)uploadState {
    ...
    if (_uploadStateDidChangeExeBlock)
        _uploadStateDidChangeExeBlock(self);
}

- (void)setUploadProgress:(float)uploadProgress {
    _uploadProgress = uploadProgress;
    if (_uploadProgressDidChangeExeBlock)
        _uploadProgressDidChangeExeBlock(self);
}
