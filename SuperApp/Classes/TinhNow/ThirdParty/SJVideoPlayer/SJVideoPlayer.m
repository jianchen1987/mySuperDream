...
+ (instancetype)lightweightPlayer {
    ...
    controlLayer.topContainerView.sjv_disappearDirection = controlLayer.leftContainerView.sjv_disappearDirection = controlLayer.bottomContainerView.sjv_disappearDirection
...
- (void)_backButtonWasTapped {
    if (self.isFullScreen && ![self _whetherToSupportOnlyOneOrientation]) {
        ...
        if (vc.navigationController.viewControllers.count > 1) {
            ...
            vc.presentingViewController ? [vc dismissViewControllerAnimated:YES completion:nil] : [vc.navigationController popViewControllerAnimated:YES];
        }
    }
}
...
- (SJVideoDefinitionSwitchingInfoObserver *)sj_switchingInfoObserver {
    if (_sj_switchingInfoObserver == nil) {
        ...
        _sj_switchingInfoObserver.statusDidChangeExeBlock = ^(SJVideoDefinitionSwitchingInfo *_Nonnull info) {
            ...
            if (!self)
                return;
            if (self.isDisabledDefinitionSwitchingPrompt)
                return;
            switch (info.status) {
                ...
                case SJDefinitionSwitchStatusSwitching: {
                    [self.promptPopupController
                        show:[NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol> _Nonnull make) {
                ...
                case SJDefinitionSwitchStatusFinished: {
                    [self.promptPopupController
                        show:[NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol> _Nonnull make) {
                            make.append([NSString
                                stringWithFormat:@"%@ %@", SJVideoPlayerConfigurations.shared.localizedStrings.definitionSwitchSuccessfullyPrompt, info.currentPlayingAsset.definition_fullName]);
                            make.textColor(UIColor.whiteColor);
                        }]];
                } break;
...
- (BOOL)_whetherToSupportOnlyOneOrientation {
    if (self.rotationManager.autorotationSupportedOrientations == SJOrientationMaskPortrait)
        return YES;
    if (self.rotationManager.autorotationSupportedOrientations == SJOrientationMaskLandscapeLeft)
        return YES;
    if (self.rotationManager.autorotationSupportedOrientations == SJOrientationMaskLandscapeRight)
        return YES;
    return NO;
}
...
- (void)_updateContentForDefinitionItemIfNeeded {
    if (self.definitionURLAssets.count != 0) {
        ...
        self.definitionItem.title = [NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol> _Nonnull make) {
            ...
            if (self.definitionSwitchingInfo.switchingAsset != nil && self.definitionSwitchingInfo.status != SJDefinitionSwitchStatusFailed) {
