...
static SJRunLoopTaskQueue *sj_queue(void) {
...
- (void)sj_playAssetAtIndexPath:(NSIndexPath *)indexPath scrollAnimated:(BOOL)animated {
    ...
    if ([self isKindOfClass:UITableView.class]) {
        ...
        UICollectionViewScrollPosition position =
            [self sj_autoplayConfig].animationType != SJAutoplayScrollAnimationTypeTop ? UICollectionViewScrollPositionCenteredVertically : UICollectionViewScrollPositionTop;
        ...
        case SJAutoplayPositionTop: {
            for (NSIndexPath *indexPath in visibleIndexPaths) {
                ...
                if (config.playerSuperviewSelector != NULL) {
                    ...
                    target = superviewTag != 0 ? [scrollView viewWithTag:superviewTag atIndexPath:indexPath] :
...
static void sj_exeAnima(__kindof UIScrollView *scrollView, NSIndexPath *indexPath, SJAutoplayScrollAnimationType animationType) {
    switch (animationType) {
        case SJAutoplayScrollAnimationTypeNone:
            break;
...
#pragma clang diagnostic pop
    for (NSIndexPath *indexPath in remain) {
        ...
        if (config.playerSuperviewSelector != NULL) {
            ...
            target = superviewTag != 0 ? [scrollView viewWithTag:superviewTag atIndexPath:indexPath] : [scrollView viewWithProtocol:@protocol(SJPlayModelPlayerSuperview) tag:0 atIndexPath:indexPath];
        }
        if (!target)
            continue;
        CGRect intersection = [scrollView intersectionWithView:target insets:config.playableAreaInsets];
...
static void sj_playAsset(UIScrollView *scrollView, NSIndexPath *indexPath, BOOL animated) {
    ...
    if (animated)
        sj_exeAnima(scrollView, indexPath, [scrollView sj_autoplayConfig].animationType);
}
