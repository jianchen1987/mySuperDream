...
- (void)pictureInPictureController:(id<SJPictureInPictureController>)controller
    restoreUserInterfaceForPictureInPictureStopWithCompletionHandler:(void (^)(BOOL))completionHandler API_AVAILABLE(ios(14.0)) {
...
- (void)screenshotWithTime:(NSTimeInterval)time size:(CGSize)size completion:(void (^)(SJAVMediaPlaybackController *controller, UIImage *__nullable image, NSError *__nullable error))block {
...
- (void)generateGIFWithBeginTime:(NSTimeInterval)beginTime
    ...
    [self cancelGenerateGIFOperation];
    __weak typeof(self) _self = self;
    [self.currentPlayer.avPlayer.currentItem.asset sj_generateGIFWithBeginTime:beginTime duration:duration imageMaxSize:maximumSize interval:interval toFile:gifSavePath
        progress:^(AVAsset *_Nonnull a, float progress) {
            __strong typeof(_self) self = _self;
            if (!self)
                return;
            if (progressBlock)
