...
- (void)sj_loadDurationOfAsset:(void (^)(CMTime duration))completionHandler {
    if (0 != CMTimeCompare(kCMTimeZero, _asset.duration)) {
        if (completionHandler)
            completionHandler(_asset.duration);
    } else {
        ...
        [_asset loadValuesAsynchronouslyForKeys:@[@"duration"] completionHandler:^{
            ...
            if (!self)
                return;
            if (completionHandler)
                completionHandler(self.asset.duration);
        }];
    }
}
...
- (void)generatePreviewImagesWithMaxItemSize:(CGSize)itemSize count:(NSUInteger)count completionHandler:(void (^)(AVAsset *_Nonnull, NSArray<UIImage *> *_Nullable, NSError *_Nullable))block {
    ...
    [self sj_loadDurationOfAsset:^(CMTime duration) {
        ...
        [self.imageGenerator
            generateCGImagesAsynchronouslyForTimes:timesM
...
- (void)exportWithStartTime:(NSTimeInterval)startTime
    ...
    if (endTime - startTime <= 0) {
        if (failure)
            failure(self.asset, [NSError errorWithDomain:NSCocoaErrorDomain code:-1 userInfo:@{@"msg": @"Error: Start time is greater than end time!"}]);
        return;
    }
    if (!presetName)
    ...
    if (error) {
        NSLog(@"Export Failed: error = %@", error);
        if (failure)
            failure(self.asset, error);
        return;
    }
    ...
    self.exportProgressRefreshTimer = [NSTimer assetAdd_timerWithTimeInterval:0.1 block:^(NSTimer *timer) {
        ...
        if (progress)
            progress(self.asset, self.exportSession.progress);
    } repeats:YES];
    ...
    [self.exportSession exportAsynchronouslyWithCompletionHandler:^{
            ...
            case AVAssetExportSessionStatusCompleted: {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (!self)
                        return;
                    if (progress)
                        progress(self.asset, 1);
                    if (success)
                        success(self.asset, compositionM, fileURL, image);
                });
            } break;
        ...
        // clear timer & session
        if (self.exportSession.status == AVAssetExportSessionStatusCancelled || self.exportSession.status == AVAssetExportSessionStatusCompleted
...
- (void)sj_screenshotWithTime:(NSTimeInterval)t size:(CGSize)size completionHandler:(void (^)(AVAsset *a, UIImage *__nullable image, NSError *__nullable error))block {
        ...
        generateCGImagesAsynchronouslyForTimes:@[[NSValue valueWithCMTime:time]]
                             completionHandler:^(CMTime requestedTime, CGImageRef _Nullable imageRef, CMTime actualTime, AVAssetImageGeneratorResult result, NSError *_Nullable error) {
                                 ...
                                 if (result == AVAssetImageGeneratorSucceeded) {
                                     if (block)
                                         block(self.asset, [UIImage imageWithCGImage:imageRef], nil);
...
- (void)sj_generateGIFWithBeginTime:(NSTimeInterval)beginTime
    ...
    int all = count;
    __weak typeof(self) _self = self;
    [self.generator generateCGImagesAsynchronouslyForTimes:timesM
                                         completionHandler:^(CMTime requestedTime, CGImageRef _Nullable imageRef, CMTime actualTime, AVAssetImageGeneratorResult result, NSError *_Nullable error) {
                                             __strong typeof(_self) self = _self;
                                             if (!self)
                                                 return;
                                                 ...
                                                 case AVAssetImageGeneratorSucceeded: {
                                                     ...
                                                     dispatch_async(dispatch_get_main_queue(), ^{
                                                         ...
                                                         if (!self)
                                                             return;
                                                         if (!result) {
                                                             NSError *error = [NSError errorWithDomain:NSCocoaErrorDomain code:-1 userInfo:@{@"msg": @"Generate Gif Failed!"}];
                                                             if (failureBlock)
                                                                 failureBlock(self.asset, error);
                                                         } else {
                                                             if (progressBlock)
                                                                 progressBlock(self.asset, 1);
                                                             if (successBlock)
                                                                 successBlock(self.asset, image, self.creator.firstImage);
                                                             self.creator = nil;
                                                         }
                                                     });
                                                 } break;
...
- (void)sj_screenshotWithTime:(NSTimeInterval)time completionHandler:(void (^)(AVAsset *a, UIImage *__nullable images, NSError *__nullable error))block {
    [self sj_screenshotWithTime:time size:CGSizeZero completionHandler:block];
}

- (void)sj_screenshotWithTime:(NSTimeInterval)time size:(CGSize)size completionHandler:(void (^)(AVAsset *a, UIImage *__nullable image, NSError *__nullable error))block {
