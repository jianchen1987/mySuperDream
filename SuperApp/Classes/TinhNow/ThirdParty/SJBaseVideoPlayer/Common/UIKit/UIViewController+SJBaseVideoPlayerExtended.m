...
typedef NS_ENUM(NSUInteger, SJModalAction) { SJModalActionPresented = 0, SJModalActionDismissed = 1 };
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext {
    switch (self.state) {
        case SJModalActionPresented: {
            ...
            if (self.presentedAnimationHandler)
                self.presentedAnimationHandler(self.modalViewController, ^{
                    [transitionContext completeTransition:YES];
                });
        } break;
        case SJModalActionDismissed: {
            if (self.dismissedAnimationHandler)
                self.dismissedAnimationHandler(self.modalViewController, ^{
                    [transitionContext completeTransition:YES];
                });
        } break;
        default:
