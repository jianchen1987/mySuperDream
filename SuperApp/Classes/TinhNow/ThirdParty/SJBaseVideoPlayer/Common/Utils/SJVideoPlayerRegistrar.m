...
- (instancetype)init {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [self sj_observeWithNotification:AVAudioSessionRouteChangeNotification target:nil usingBlock:^(SJVideoPlayerRegistrar *_Nonnull self, NSNotification *_Nonnull note) {
            ...
            switch (routeChangeReason) {
                case AVAudioSessionRouteChangeReasonNewDeviceAvailable: {
                    if (self.newDeviceAvailable)
                        self.newDeviceAvailable(self);
                } break;
                case AVAudioSessionRouteChangeReasonOldDeviceUnavailable: {
                    if (self.oldDeviceUnavailable)
        ...
        [self sj_observeWithNotification:AVAudioSessionInterruptionNotification target:[AVAudioSession sharedInstance]
                              usingBlock:^(SJVideoPlayerRegistrar *_Nonnull self, NSNotification *_Nonnull note) {
                                  ...
                                  if ((AVAudioSessionInterruptionType)[info[AVAudioSessionInterruptionTypeKey] integerValue] == AVAudioSessionInterruptionTypeBegan) {
                                      if (self.audioSessionInterruption)
