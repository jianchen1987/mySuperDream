...
- (instancetype)initWithInfo:(SJVideoDefinitionSwitchingInfo *)info {
    ...
    if (self) {
        ...
        _token = [NSNotificationCenter.defaultCenter addObserverForName:_SJVideoDefinitionSwitchStatusDidChangeNotification object:info queue:NSOperationQueue.mainQueue
                                                             usingBlock:^(NSNotification *_Nonnull note) {
                                                                 __strong typeof(_self) self = _self;
                                                                 if (!self)
                                                                     return;
                                                                 if (self.statusDidChangeExeBlock)
                                                                     self.statusDidChangeExeBlock(note.object);
                                                             }];
    }
    return self;
}
