...
- (void)setFitOnScreen:(BOOL)fitOnScreen animated:(BOOL)animated completionHandler:(nullable void (^)(id<SJFitOnScreenManager>))completionHandler {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.isTransitioning)
            return;
        if (fitOnScreen == self.isFitOnScreen) {
        ...
        if (fitOnScreen) {
            ...
            if (!animated)
                [self _presentedAnimationWithDuration:0 completionHandler:nil];
            ...
            if (!animated)
                [self _dismissedAnimationWithDuration:0 completionHandler:nil];
