...
- (instancetype)initWithMgr:(id<SJDeviceVolumeAndBrightnessManager>)mgr {
    ...
    _volumeDidChangeToken = [NSNotificationCenter.defaultCenter addObserverForName:SJDeviceVolumeDidChangeNotification object:mgr queue:NSOperationQueue.mainQueue
                                                                        usingBlock:^(NSNotification *_Nonnull note) {
                                                                            __strong typeof(_self) self = _self;
                                                                            if (!self)
                                                                                return;
                                                                            id<SJDeviceVolumeAndBrightnessManager> mgr = note.object;
                                                                            if (self.volumeDidChangeExeBlock)
                                                                                self.volumeDidChangeExeBlock(mgr, mgr.volume);
                                                                        }];

    _brightnessDidChangeToken = [NSNotificationCenter.defaultCenter addObserverForName:SJDeviceBrightnessDidChangeNotification object:mgr queue:NSOperationQueue.mainQueue
                                                                            usingBlock:^(NSNotification *_Nonnull note) {
                                                                                __strong typeof(_self) self = _self;
                                                                                if (!self)
                                                                                    return;
                                                                                id<SJDeviceVolumeAndBrightnessManager> mgr = note.object;
                                                                                if (self.brightnessDidChangeExeBlock)
...
- (void)dealloc {
    if (_volumeDidChangeToken)
        [NSNotificationCenter.defaultCenter removeObserver:_volumeDidChangeToken];
    if (_brightnessDidChangeToken)
        [NSNotificationCenter.defaultCenter removeObserver:_brightnessDidChangeToken];
}
@end
...
- (MPVolumeView *)sysVolumeView {
    if (_sysVolumeView == nil) {
        CGFloat maxOffset = MAX(CGRectGetWidth(UIScreen.mainScreen.bounds), CGRectGetHeight(UIScreen.mainScreen.bounds)) + 100;
...
- (void)_showVolumeViewIfNeeded {
    ...
    [self performSelector:@selector(_hiddenVolumeView) withObject:nil afterDelay:1 inModes:@[NSRunLoopCommonModes]];
}
...
- (void)_showBrightnessViewIfNeeded {
    ...
    [self performSelector:@selector(_hiddenBrightnessView) withObject:nil afterDelay:1 inModes:@[NSRunLoopCommonModes]];
}
