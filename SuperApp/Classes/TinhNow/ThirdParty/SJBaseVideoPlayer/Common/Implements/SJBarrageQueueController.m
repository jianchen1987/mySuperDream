...
- (void)_pausedDidChange:(NSNotification *)note {
    if (_pausedDidChangeExeBlock)
        _pausedDidChangeExeBlock(note.object);
}
- (void)_disabledDidChange:(NSNotification *)note {
    if (_disabledDidChangeExeBlock)
        _disabledDidChangeExeBlock(note.object);
}
- (void)_willDisplayBarrage:(NSNotification *)note {
    if (_willDisplayBarrageExeBlock)
        _willDisplayBarrageExeBlock(note.object, note.userInfo[SJBarrageQueueControllerBarrageItemKey]);
}
- (void)_didEndDisplayBarrage:(NSNotification *)note {
    if (_didEndDisplayBarrageExeBlock)
