...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    ...
    if (new == UIDeviceOrientationLandscapeLeft || new == UIDeviceOrientationLandscapeRight) {
...
- (BOOL)isFullscreen {
    return _currentOrientation == (NSInteger)UIDeviceOrientationLandscapeLeft || _currentOrientation == (NSInteger)UIDeviceOrientationLandscapeRight;
}
...
- (void)rotate {
    if (![self _isSupported:SJOrientation_LandscapeLeft] && ![self _isSupported:SJOrientation_LandscapeRight]) {
...
- (void)_beginTransition {
    ...
    if (!_forcedRotation)
        _window.fullscreenModeViewController.disableAnimations = NO; // 自动旋转时, 默认触发动画
}
