...
- (void)handlePan:(UIPanGestureRecognizer *)pan {
    ...
    switch (pan.state) {
        case UIGestureRecognizerStateBegan: {
            if (_panHandler)
                _panHandler(self, _triggeredPosition, _movingDirection, SJPanGestureRecognizerStateBegan, translate);
        } break;
        case UIGestureRecognizerStateChanged: {
            if (_panHandler)
                _panHandler(self, _triggeredPosition, _movingDirection, SJPanGestureRecognizerStateChanged, translate);
        } break;
        case UIGestureRecognizerStateFailed:
        case UIGestureRecognizerStateCancelled:
        case UIGestureRecognizerStateEnded: {
            if (_panHandler)
                _panHandler(self, _triggeredPosition, _movingDirection, SJPanGestureRecognizerStateEnded, translate);
        } break;
        default:
            break;
    }
    ...
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan: {
            if (_longPressHandler)
                _longPressHandler(self, SJLongPressGestureRecognizerStateBegan);
        } break;
        case UIGestureRecognizerStateChanged: {
            if (_longPressHandler)
                _longPressHandler(self, SJLongPressGestureRecognizerStateChanged);
        } break;
        case UIGestureRecognizerStateEnded:
        case UIGestureRecognizerStateCancelled:
        case UIGestureRecognizerStateFailed: {
            if (_longPressHandler)
                _longPressHandler(self, SJLongPressGestureRecognizerStateEnded);
        } break;
        default:
            break;
    }
...
- (void)hiddenPlaceholderAnimated:(BOOL)animated delay:(NSTimeInterval)secs {
    ...
    if (secs == 0) {
        ...
        [self performSelector:@selector(_hiddenPlaceholderAnimated:) withObject:@(animated) afterDelay:secs inModes:@[NSRunLoopCommonModes]];
    }
}
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    ...
    if (otherGestureRecognizer != _pan && otherGestureRecognizer != _pinch)
        return NO;
