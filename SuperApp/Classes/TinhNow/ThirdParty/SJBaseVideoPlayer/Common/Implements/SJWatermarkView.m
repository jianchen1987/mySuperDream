...
- (void)layoutWatermarkInRect:(CGRect)rect videoPresentationSize:(CGSize)vSize videoGravity:(SJVideoGravity)videoGravity {
    ...
    self.hidden = CGSizeEqualToSize(vSize, CGSizeZero) || CGSizeEqualToSize(rect.size, CGSizeZero) || CGSizeEqualToSize(imageSize, CGSizeZero);
    if (self.isHidden)
        return;
    ...
    if (videoGravity == AVLayerVideoGravityResizeAspect) {
        ...
        videoDisplayedSize
            = vSize.width > vSize.height ? CGSizeMake(rect.size.width, vSize.height * rect.size.width / vSize.width) : CGSizeMake(vSize.width * rect.size.height / vSize.height, rect.size.height);
    } else if (videoGravity == AVLayerVideoGravityResizeAspectFill) {
        // 填充模式
        // 16/9 的会将宽度进行等比拉伸, 以显示全部高度
        // 9/16 的会将高度进行等比拉伸, 以显示全部宽度
        videoDisplayedSize
            = vSize.width > vSize.height ? CGSizeMake(vSize.width * rect.size.height / vSize.height, rect.size.height) : CGSizeMake(rect.size.width, vSize.height * rect.size.width / vSize.width);
