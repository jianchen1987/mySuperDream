...
- (instancetype)initWithInterval:(NSTimeInterval)interval
                               player:(__weak id<SJMediaPlayer>)player
         currentTimeDidChangeExeBlock:(nonnull void (^)(NSTimeInterval time))currentTimeDidChangeExeBlock
    playableDurationDidChangeExeBlock:(nonnull void (^)(NSTimeInterval time))playableDurationDidChangeExeBlock
            durationDidChangeExeBlock:(nonnull void (^)(NSTimeInterval time))durationDidChangeExeBlock;
- (void)invalidate;
- (void)stop;
@end
...
- (instancetype)initWithInterval:(NSTimeInterval)interval
                               player:(__weak id<SJMediaPlayer>)player
         currentTimeDidChangeExeBlock:(nonnull void (^)(NSTimeInterval))currentTimeDidChangeExeBlock
    playableDurationDidChangeExeBlock:(nonnull void (^)(NSTimeInterval))playableDurationDidChangeExeBlock
            durationDidChangeExeBlock:(nonnull void (^)(NSTimeInterval))durationDidChangeExeBlock {
    self = [super init];
    if (self) {
        _interval = interval;
        _player = player;
...
- (void)durationDidChange {
    if (_durationDidChangeExeBlock)
        _durationDidChangeExeBlock(_player.duration);
}

- (void)playableDurationDidChange {
    if (_playableDurationDidChangeExeBlock)
        _playableDurationDidChangeExeBlock(_player.playableDuration);
}
...
- (void)stop {
    [self invalidate];
    if (_playableDurationDidChangeExeBlock)
        _playableDurationDidChangeExeBlock(0);
    if (_currentTimeDidChangeExeBlock)
        _currentTimeDidChangeExeBlock(0);
    if (_durationDidChangeExeBlock)
        _durationDidChangeExeBlock(0);
}
...
- (void)_refresh {
    ...
    if (_currentTime != currentTime) {
        ...
        if (_currentTimeDidChangeExeBlock)
            _currentTimeDidChangeExeBlock(currentTime);
    }
}
@end
...
- (void)play {
    ...
    // play
        ...
        if (self.currentPlayer.rate != self.rate)
            self.currentPlayer.rate = self.rate;
        [self _toEvaluating];
    }
}
...
- (void)switchVideoDefinition:(SJVideoPlayerURLAsset *)media {
    ...
    [self playerWithMedia:media completionHandler:^(id<SJMediaPlayer> _Nullable player) {
        ...
        self.definitionMediaPlayerLoader = [SJDefinitionMediaPlayerLoader.alloc initWithDefinitionMediaPlayer:definitionMediaPlayer definitionMediaPlayerView:definitionMediaPlayerView
                                                                                                currentPlayer:currentPlayer
                                                                                            currentPlayerView:currentPlayerView
                                                                                            completionHandler:^(SJDefinitionMediaPlayerLoader *_Nonnull loader, BOOL isFinished) {
                                                                                                __strong typeof(_self) self = _self;
                                                                                                if (!self)
                                                                                                    return;
                                                                                                if (media != self.definitionMedia)
...
- (void)_toEvaluating {
    ...
    if (self.timeControlStatus != self.currentPlayer.timeControlStatus || self.reasonForWaitingToPlay != self.currentPlayer.reasonForWaitingToPlay) {
...
- (void)_didCompleteLoad:(BOOL)result {
    ...
    if (_completionHandler)
        _completionHandler(self, result);
