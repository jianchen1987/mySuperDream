...
- (NSString *)description {
    NSString *description = [NSString stringWithFormat:@"<%@: %p (%@)>", NSStringFromClass([self class]), self, [self currentReachabilityFlags]];
    return description;
}
...
- (void)networkSpeedDidChange:(NSNotification *)note {
    if (_networkSpeedDidChangeExeBlock)
        _networkSpeedDidChangeExeBlock(_reachability);
}
@end
