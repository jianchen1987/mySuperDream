...
NSString *sj_sqlite3_obj_get_default_table_name(Class cls) {
    return [NSString stringWithFormat:@"%s", object_getClassName(cls)];
}

id sj_sqlite3_obj_filter_obj_value(id value) {
...
///
NSString *sj_sqlite3_stmt_create_table(SJSQLiteTableInfo *table) {
    ...
    [sql appendFormat:@"CREATE TABLE %@ (", table.name];
    {
        for (SJSQLiteColumnInfo *column in table.columns) {
            ...
            if (column.constraints)
                [sql appendFormat:@" %@", column.constraints];
            if (column != last)
                [sql appendString:@","];
        }
    }
...
///
NSString *sj_sqlite3_stmt_insert_or_update(SJSQLiteObjectInfo *objInfo) {
    ...
    for (SJSQLiteColumnInfo *column in columns) {
        ...
        if (column.isAutoincrement && [value integerValue] == 0)
            continue;
...
NSString *sj_sqlite3_stmt_get_column_value(SJSQLiteColumnInfo *column, id value) {
    NSString *data = nil;
...
NSString *_Nullable sj_sqlite3_stmt_get_primary_values_json_string(NSArray *models, NSString *primaryKey) {
...
NSArray<id> *_Nullable sj_sqlite3_stmt_get_primary_values_array(NSString *jsonString) {
    NSData *data = [jsonString dataUsingEncoding:NSUTF8StringEncoding];
    return [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
}

NSString *sj_sqlite3_stmt_get_last_row(SJSQLiteTableInfo *table) {
...
///
int sj_sqlite3_obj_exec_each_result_callback(void *para, int ncolumn, char **columnvalue, char **columnname) {
    NSMutableArray<NSDictionary *> *results = (__bridge NSMutableArray *)para;
    NSMutableDictionary *result = NSMutableDictionary.new;
    for (int i = 0; i < ncolumn; ++i) {
        char *_Nullable value = columnvalue[i];
        if (value)
            result[[NSString stringWithUTF8String:columnname[i]]] = [NSString stringWithUTF8String:value];
    }
...
/// 打开数据库链接
///
BOOL sj_sqlite3_obj_open_database(NSString *path, void *db) {
...
/// 关闭数据库链接
///
BOOL sj_sqlite3_obj_close_database(void *db) {
    return sqlite3_close(db);
}

/// 执行sql
///
NSArray<NSDictionary *> *_Nullable sj_sqlite3_obj_exec(void *db, NSString *sql, NSError *_Nullable *_Nullable error) {
    if (sql.length == 0)
        return nil;
...
/// 开启事物
///
void sj_sqlite3_obj_begin_transaction(void *db) {
    sj_sqlite3_obj_exec(db, @"BEGIN TRANSACTION", nil);
}

/// 提交事物
///
void sj_sqlite3_obj_commit(void *db) {
    sj_sqlite3_obj_exec(db, @"COMMIT", nil);
}

/// 回滚提交
///
void sj_sqlite3_obj_rollback(void *db) {
    sj_sqlite3_obj_exec(db, @"ROLLBACK", nil);
}

/// 查询某个表是否存在
///
BOOL sj_sqlite3_obj_table_exists(void *db, NSString *name) {
    return sj_sqlite3_obj_exec(db, [NSString stringWithFormat:@"SELECT tbl_name FROM sqlite_master WHERE name='%@';", name], nil) != nil;
}

/// 删除表
///
void sj_sqlite3_obj_drop_table(void *db, NSString *name, NSError **error) {
...
/// 删除指定的行数据
///
void sj_sqlite3_obj_delete_row_datas(void *db, SJSQLiteTableInfo *table, NSArray<id> *primaryKeyValues, NSError **error) {
    NSMutableString *values = NSMutableString.new;
...
/// 获取行数据
///
NSDictionary *_Nullable sj_sqlite3_obj_get_row_data(void *db, SJSQLiteTableInfo *table, id primaryKeyValue, NSError **error) {
