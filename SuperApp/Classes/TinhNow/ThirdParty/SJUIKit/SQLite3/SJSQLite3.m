...
#define SJSQLite3_TRANSACTION_BEGIN() \
    SJSQLite3_Lock();                 \
    sj_sqlite3_obj_begin_transaction(self.db);

#define SJSQLite3_TRANSACTION_ROLLBACK() \
    sj_sqlite3_obj_rollback(self.db);    \
    SJSQLite3_Unlock();

#define SJSQLite3_TRANSACTION_COMMIT() \
    sj_sqlite3_obj_commit(self.db);    \
    SJSQLite3_Unlock();
...
- (BOOL)saveObjects:(NSArray *)objectArray error:(NSError **)error {
    if (objectArray.count == 0) {
        if (error != NULL)
            *error = sqlite3_error_invalid_parameter();
        return NO;
    }
...
- (nullable NSError *)_insertOrUpdateObjects:(NSArray *)objsArray {
    ...
    for (id obj in objsArray) {
        ...
        for (Class cls in objectInfo.table.allClasses) {
            if (![self _checkoutAllTablesForClass:cls error:&error])
                return error;
        }
...
- (BOOL)_checkoutAllTablesForClass:(Class)cls error:(NSError **)error {
    if ([self.tableClassCache containsClass:cls])
        return YES;
    ...
    for (Class cls in tableInfo.allClasses) {
        ...
        if ([self _containsTableForClass:cls]) {
            if (![self _alterTableIfNeeded:cls error:&inner_error])
                break;
        } else {
            if (![self _createTableForClass:cls error:&inner_error])
                break;
        }
    }
