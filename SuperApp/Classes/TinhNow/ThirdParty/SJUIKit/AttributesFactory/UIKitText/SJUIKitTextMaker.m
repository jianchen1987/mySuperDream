...
- (void)addAttributesForRecorder:(SJUTRecorder *)recorder range:(NSRange)subrange {
    ...
    if (textAttributes.count != 0)
        [self addAttributes:textAttributes range:subrange];
    ...
    if (customAttributes.count != 0)
        [self addAttributes:customAttributes range:subrange];
}
@end
...
- (NSMutableAttributedString *)install {
    ...
    if (recorder->font == nil)
        recorder->font = [UIFont systemFontOfSize:14];
    if (recorder->textColor == nil)
        recorder->textColor = [UIColor blackColor];
...
- (void)_executeRegexHandlersIfNeeded:(NSMutableAttributedString *)result {
    if (_regexs) {
        for (SJUTRegexHandler *handler in _regexs) {
            ...
            [regular enumerateMatchesInString:string options:recorder.matchingOptions range:resultRange
