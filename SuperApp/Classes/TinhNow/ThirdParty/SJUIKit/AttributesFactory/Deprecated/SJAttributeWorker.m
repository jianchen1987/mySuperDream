...
- (void)removeAttributeWithKey:(NSAttributedStringKey)attributedStringKey {
    if (attributedStringKey == NSFontAttributeName)
        _recorder.font = nil;
    else if (attributedStringKey == NSForegroundColorAttributeName)
        _recorder.textColor = nil;
    else if (attributedStringKey == NSExpansionAttributeName)
        _recorder.expansion = 0;
    else if (attributedStringKey == NSShadowAttributeName)
        _recorder.shadow = nil;
    else if (attributedStringKey == NSBackgroundColorAttributeName)
        _recorder.backgroundColor = nil;
    else if (attributedStringKey == NSUnderlineStyleAttributeName)
        _recorder.underLine = nil;
    else if (attributedStringKey == NSStrikethroughStyleAttributeName)
        _recorder.strikethrough = nil;
    else if (attributedStringKey == NSStrokeWidthAttributeName)
        _recorder.stroke = nil;
    else if (attributedStringKey == NSObliquenessAttributeName)
        _recorder.obliqueness = 0;
    else if (attributedStringKey == NSKernAttributeName)
        _recorder.letterSpacing = 0;
    else if (attributedStringKey == NSBaselineOffsetAttributeName)
        _recorder.offset = 0;
    else if (attributedStringKey == NSLinkAttributeName)
        _recorder.link = NO;
    else if (attributedStringKey == NSParagraphStyleAttributeName)
        _recorder.paragraphStyleM = [NSParagraphStyle defaultParagraphStyle].mutableCopy;
}
@end
...
- (void)_addCommonValuesToRecorderIfNeed:(SJAttributesRecorder *)recorder {
    if (nil == recorder.font)
        recorder.font = self.defaultFont;
    if (nil == recorder.textColor)
        recorder.textColor = self.defaultTextColor;
    if (0 == recorder.lineSpacing)
        recorder.lineSpacing = self.recorder.lineSpacing;
    if (nil == recorder.alignment)
        recorder.alignment = self.recorder.alignment;
}
    ...
    [self.rangeOperatorsM enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(SJAttributesRangeOperator *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
        ...
        if (_rangeContains(insertingRange, objRange)) {
            ...
            if (leftRange.length != 0)
                [self _getOperatorWithRange:leftRange];
...
- (void)_adjustOperatorsWhenReplaceCharactersInRange:(NSRange)range_old textLength:(NSInteger)textLength {
    ...
    if (NSEqualRanges(range_old, range_new))
        return;
...
- (SJAttributeWorker *_Nonnull (^)(NSString *_Nonnull, void (^_Nonnull)(SJAttributesRangeOperator *_Nonnull)))regexp {
    return ^SJAttributeWorker *(NSString *regStr, void (^task)(SJAttributesRangeOperator *matched)) {
        return self.regexp_r(
            regStr, ^(NSArray<NSValue *> *_Nonnull matchedRanges) {
...
- (SJAttributeWorker *_Nonnull (^)(NSString *_Nonnull, void (^_Nonnull)(NSArray<NSValue *> *_Nonnull), BOOL reverse))regexp_r {
    return ^SJAttributeWorker *(NSString *regStr, void (^task)(NSArray<NSValue *> *ranges), BOOL reverse) {
        ...
        [regex enumerateMatchesInString:self->_attrStr.string options:NSMatchingWithoutAnchoringBounds range:self.range
        ...
        if (reverse) {
            ...
            [rangesM enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSValue *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
                [reverseM addObject:obj];
            }];
            rangesM = reverseM;
        }
        task(rangesM);
...
- (void (^)(NSString *_Nonnull, id _Nonnull, ...))regexp_replace {
    return ^(NSString *regexp, id replaceByStrOrAttrStrOrImg, ...) {
        ...
        self.regexp_r(
            regexp, ^(NSArray<NSValue *> *_Nonnull matchedRanges) {
                [matchedRanges enumerateObjectsUsingBlock:^(NSValue *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
                    if ([replaceByStrOrAttrStrOrImg isKindOfClass:[NSString class]] || [replaceByStrOrAttrStrOrImg isKindOfClass:[NSAttributedString class]]) {
