...
- (instancetype)initWithTarget:(__unsafe_unretained id)target forKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options change:(SJKVOObservedChangeHandler)hanlder {
    self = [super init];
    if (!self)
        return nil;
    _target = target;
...
- (void)observeValueForKeyPath:(nullable NSString *)keyPath ofObject:(nullable id)object change:(nullable NSDictionary<NSKeyValueChangeKey, id> *)change context:(nullable void *)context {
    if (_hanlder)
        _hanlder(object, change);
}
@end

SJKVOObserverToken sjkvo_observe(id target, NSString *keyPath, SJKVOObservedChangeHandler handler) {
...
SJKVOObserverToken __attribute__((overloadable)) sjkvo_observe(id target, NSString *keyPath, NSKeyValueObservingOptions options, SJKVOObservedChangeHandler handler) {
    if (!target)
        return 0;
...
void sjkvo_remove(id target, SJKVOObserverToken token) {
