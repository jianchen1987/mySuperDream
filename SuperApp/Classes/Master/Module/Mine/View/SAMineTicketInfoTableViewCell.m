//
//  SAMineTicketInfoTableViewCell.m
//  SuperApp
//
...
@interface SAMineTicketInfoTableViewCell ()
/// 标题
...
@implementation SAMineTicketInfoTableViewCell
...
- (void)configCellWithModel:(SACouponInfoRspModel *)model {
    self.titleLB.text = SALocalizedString(@"my_coupons", @"我的优惠券");
    self.cashCouponLB.text = SALocalizedString(@"cash_coupons", @"现金券");
    self.discountCouponLB.text = SALocalizedString(@"discount_coupons", @"折扣券");
...
- (SALabel *)titleLB {
    if (!_titleLB) {
        ...
        label.text = SALocalizedString(@"my_coupons", @"我的优惠券");
        _titleLB = label;
    }
    return _titleLB;
}
...
- (SALabel *)cashCouponLB {
    if (!_cashCouponLB) {
        ...
        label.numberOfLines = 0;
        label.text = SALocalizedString(@"cash_coupons", @"现金券");
        _cashCouponLB = label;
    }
...
- (SALabel *)discountCouponLB {
    if (!_discountCouponLB) {
        ...
        label.textColor = HDAppTheme.color.G3;
        label.numberOfLines = 0;
        label.text = SALocalizedString(@"discount_coupons", @"折扣券");
        label.hidden = true;
