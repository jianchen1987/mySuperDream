...
@interface HDCheckStandTextField () <UIKeyInput, UITextInputTraits> {
...
@implementation HDCheckStandTextField
...
- (void)layoutSubviews {
    ...
    if (self.borderType != HDCheckStandTextFieldBorderTypeNoBorder) {
    ...
    } else if (self.borderType == HDCheckStandTextFieldBorderTypeNoBorder) {
...
- (void)setDiameterOfDot:(CGFloat)diameterOfDot {
    ...
    if (self.placeholderDic[@(HDCheckStandSecurityCharacterTypeSecurityDot)]) {
...
- (void)setColorOfCharacter:(UIColor *)colorOfCharacter {
    ...
    if (self.securityCharacterType == HDCheckStandSecurityCharacterTypeSecurityDot) {
    ...
    } else if (self.securityCharacterType == HDCheckStandSecurityCharacterTypePlainText) {
    ...
    } else if (self.securityCharacterType == HDCheckStandSecurityCharacterTypeHorizontalLine) {
        [self.placeholderDic[@(self.securityCharacterType)] enumerateObjectsUsingBlock:^(CATextLayer *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)setSecurityCharacterType:(HDCheckStandSecurityCharacterType)securityCharacterType {
    _securityCharacterType = securityCharacterType;

    if (!self.placeholderDic[@(securityCharacterType)]) {
        if (securityCharacterType == HDCheckStandSecurityCharacterTypeSecurityDot) {
            self.placeholderDic[@(securityCharacterType)] = self.dots;
        } else if (securityCharacterType == HDCheckStandSecurityCharacterTypePlainText) {
            self.placeholderDic[@(securityCharacterType)] = self.plaintexts;
        } else if (securityCharacterType == HDCheckStandSecurityCharacterTypeCustomImage) {
            self.placeholderDic[@(securityCharacterType)] = self.customImages;
        } else if (securityCharacterType == HDCheckStandSecurityCharacterTypeHorizontalLine) {
            self.placeholderDic[@(securityCharacterType)] = self.horizontalLines;
        }
    }
}

- (void)setBorderType:(HDCheckStandTextFieldBorderType)borderType {
    _borderType = borderType;

    switch (borderType) {
        case HDCheckStandTextFieldBorderTypeHaveRoundedCorner: {
            self.layer.borderColor = self.tintColor.CGColor;
            self.layer.borderWidth = DEFAULT_BODER_WIDTH;
            self.layer.cornerRadius = DEFAULT_RADIUS;
            break;
        }
        case HDCheckStandTextFieldBorderTypeNoRoundedCorner: {
            self.layer.borderColor = self.tintColor.CGColor;
            self.layer.borderWidth = DEFAULT_BODER_WIDTH;
            self.layer.cornerRadius = 0;
            break;
        }
        case HDCheckStandTextFieldBorderTypeNoBorder: {
...
- (void)observeValueForKeyPath:(nullable NSString *)keyPath ofObject:(nullable id)object change:(nullable NSDictionary<NSKeyValueChangeKey, id> *)change context:(nullable void *)context {
    if ([keyPath isEqualToString:@"frame"]) {
        if (self.securityCharacterType == HDCheckStandSecurityCharacterTypeSecurityDot || self.securityCharacterType == HDCheckStandSecurityCharacterTypeHorizontalLine) {
        } else if (self.securityCharacterType == HDCheckStandSecurityCharacterTypePlainText) {
            [self.placeholderDic[@(self.securityCharacterType)] enumerateObjectsUsingBlock:^(CALayer *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)insertText:(NSString *)text {
    if (self.text.length < self.numberOfCharacters) {
        ...
        if (self.securityCharacterType == HDCheckStandSecurityCharacterTypePlainText) {
        ...
        if (index == self.numberOfCharacters - 1) {
            completion_count++;
            if (@protocol(HDCheckStandTextFieldDelegate) && [self.delegate respondsToSelector:@selector(checkStandTextFieldDidFinishedEditing:)]) {
                [self.delegate checkStandTextFieldDidFinishedEditing:self];
            }
            !self.completion ?: self.completion(self, self.text);
        } else {
            if (@protocol(HDCheckStandTextFieldDelegate) && [self.delegate respondsToSelector:@selector(checkStandTextFieldDidBeginEditing:)]) {
...
- (void)deleteBackward {
    if (self.text.length > 0) {
        ...
        if (@protocol(HDCheckStandTextFieldDelegate) && [self.delegate respondsToSelector:@selector(checkStandTextFieldDidDelete:)]) {
            [self.delegate checkStandTextFieldDidDelete:self];
        }

        if (!self.text.length) {
            if (@protocol(HDCheckStandTextFieldDelegate) && [self.delegate respondsToSelector:@selector(checkStandTextFieldDidClear:)]) {
...
- (void)hightBoxs {
    for (int i = 0; i < self.boxs.count; i++) {
        ...
        if (i == self.text.length) {
            box.borderColor = UIColor.orangeColor.CGColor;
        } else {
            box.borderColor = UIColor.lightTextColor.CGColor;
        }
    }
}
...
- (void)darkAllBoxs {
    for (int i = 0; i < self.boxs.count; i++) {
        CALayer *box = self.boxs[i];
        box.borderColor = UIColor.blackColor.CGColor;
    }
}
...
- (BOOL)becomeFirstResponder {
    ...
    if (@protocol(HDCheckStandTextFieldDelegate) && [self.delegate respondsToSelector:@selector(checkStandTextFieldBecomeFirstResponder:)]) {
...
- (BOOL)resignFirstResponder {
    [self darkAllBoxs];
    if (@protocol(HDCheckStandTextFieldDelegate) && [self.delegate respondsToSelector:@selector(checkStandTextFieldResignFirstResponder:)]) {
