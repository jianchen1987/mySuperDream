...
- (void)commonInit {
    ...
    [self addSubview:self.subValueLabel];
    // 添加点击事件监听
...
- (void)setProperties {
    ...
    if (!self.keyButton.isHidden) {
        ...
        if (self.model.alignType == SAInfoViewAlignTypeHorizontal) {
    ...
    if (self.model.alignType == SAInfoViewAlignTypeHorizontal && HDIsStringNotEmpty(self.model.subValueText) && !self.rightButton.isHidden) {
        self.subValueLabel.hidden = NO;
        self.subValueLabel.text = self.model.subValueText;
        self.subValueLabel.font = self.model.subValueFont;
        self.subValueLabel.textColor = self.model.subValueColor;
    } else {
...
- (void)updateConstraints {
    ...
    if (self.model.alignType == SAInfoViewAlignTypeVertical) {
    ...
    [self.valueButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        if (!self.valueButton.isHidden) {
            if (self.model.alignType == SAInfoViewAlignTypeVertical) {
                ...
                if (self.subValueLabel.isHidden) {
    ...
    if (!self.subValueLabel.isHidden) {
        [self.subValueLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.trailing.equalTo(self.valueButton.mas_trailing);
            make.top.equalTo(self.valueButton.mas_bottom).offset(self.model.subToValueButtonTopSpace);
            if (self.model.needFixedBottom) {
                make.bottom.equalTo(self).offset(-self.model.contentEdgeInsets.bottom);
            } else {
                make.bottom.lessThanOrEqualTo(self).offset(-self.model.contentEdgeInsets.bottom);
            ...
            if (self.model.alignType == SAInfoViewAlignTypeVertical) {
                ...
                if (self.model.contentEdgeInsets.top != self.model.contentEdgeInsets.bottom) {
                    ...
                    if (self.valueButton.isHidden) {
                        ...
                        if (!self.subValueLabel.isHidden) {
                            make.centerY.equalTo(self.valueButton.mas_centerY);
                        } else {
                            make.centerY.equalTo(self);
                        }
                    }
...
/** @lazy subValueLabel */
- (UILabel *)subValueLabel {
    if (!_subValueLabel) {
        _subValueLabel = [[UILabel alloc] init];
        _subValueLabel.numberOfLines = 0;
        _subValueLabel.textAlignment = NSTextAlignmentRight;
    }
    return _subValueLabel;
}
- (UIView *)bottomLine {
