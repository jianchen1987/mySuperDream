///
//  HDCollectionViewVerticalLayout.m
//  SuperApp
//
...
@interface HDCollectionViewVerticalLayout ()

@end


@implementation HDCollectionViewVerticalLayout
    ...
    for (int index = 0; index < sectionCount; index++) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(collectionView:layout:registerBackView:)]) {
            ...
            if (className != nil && className.length > 0) {
                ...
                [self registerClass:[HDCollectionReusableView class] forDecorationViewOfKind:@"HDCollectionReusableView"];
            }
        } else {
            [self registerClass:[HDCollectionReusableView class] forDecorationViewOfKind:@"HDCollectionReusableView"];
        }
        ...
        if (headerH > 0) {
            ...
            HDCollectionViewLayoutAttributes *headerAttr = [HDCollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionHeader
        ...
        if (itemCount > 0) {
            ...
            if (self.layoutType == HDCollectionLayoutTypeClosest) {
            ...
            for (int i = 0; i < itemCount; i++) {
                ...
                HDCollectionViewLayoutAttributes *attributes = [HDCollectionViewLayoutAttributes layoutAttributesForCellWithIndexPath:indexPath];

                NSInteger preRow = self.attributesArray.count - 1;
                switch (self.layoutType) {
#pragma mark - 标签布局处理
                    case HDCollectionLayoutTypeLabel: {
                        //找上一个cell
                        if (preRow >= 0) {
                            if (i > 0) {
                                HDCollectionViewLayoutAttributes *preAttr = self.attributesArray[preRow];
...
#pragma mark - 列布局处理
                    case HDCollectionLayoutTypeClosest: {
...
#pragma mark - 百分比布局处理
                    case HDCollectionLayoutTypePercent: {
                        ...
                        if (arrayOfPercent.count > 0) {
                            ...
                            if ((totalPercent + percent) >= 1.0) {
                                if ((totalPercent + percent) < 1.1) {
                                    ...
                                    attributes.frame = CGRectMake(0,
                                                                  0,
                                                                  (itemSize.width > self.collectionView.frame.size.width - edgeInsets.left - edgeInsets.right) ?
                                                                      self.collectionView.frame.size.width - edgeInsets.left - edgeInsets.right :
                                                                      itemSize.width,
                                                                  itemSize.height);
                                    // 再添加进计算比例的数组
                                    [arrayOfPercent
                                        addObject:[NSMutableDictionary dictionaryWithDictionary:@{@"item": attributes, @"percent": [NSNumber numberWithFloat:percent], @"indexPath": indexPath}]];
                                    ...
                                    for (NSInteger i = 0; i < arrayOfPercent.count; i++) {
                                        ...
                                        HDCollectionViewLayoutAttributes *newAttributes = dic[@"item"];
                                        CGFloat itemX = 0.0f;
                                        if (i == 0) {
                                            itemX = edgeInsets.left;
                                        } else {
                                            HDCollectionViewLayoutAttributes *preAttr = arrayOfPercent[i - 1][@"item"];
                                            itemX = preAttr.frame.origin.x + preAttr.frame.size.width + minimumInteritemSpacing;
                                        }
                                        newAttributes.frame = CGRectMake(itemX,
                                                                         (maxYOfPercent == -1) ? y : maxYOfPercent + minimumLineSpacing,
                                                                         realWidth * [dic[@"percent"] floatValue],
                                                                         newAttributes.frame.size.height);
                                    ...
                                    for (NSInteger i = 0; i < arrayOfPercent.count; i++) {
                                        NSDictionary *dic = arrayOfPercent[i];
                                        HDCollectionViewLayoutAttributes *item = dic[@"item"];
                                    ...
                                    if (arrayOfPercent.count > 0) {
                                        for (int i = 0; i < arrayOfPercent.count; i++) {
                                            NSDictionary *dic = arrayOfPercent[i];
                                            HDCollectionViewLayoutAttributes *attr = dic[@"item"];
                                    ...
                                    attributes.indexPath = indexPath;
                                    attributes.frame = CGRectMake(edgeInsets.left,
                                                                  maxYOfPercent,
                                                                  (itemSize.width > self.collectionView.frame.size.width - edgeInsets.left - edgeInsets.right) ?
                                                                      self.collectionView.frame.size.width - edgeInsets.left - edgeInsets.right :
                                                                      itemSize.width,
                                ...
                                HDCollectionViewLayoutAttributes *lastAttributesForPercent = lastDicForPercent[@"item"];
                                attributes.frame = CGRectMake(lastAttributesForPercent.frame.origin.x + lastAttributesForPercent.frame.size.width + minimumInteritemSpacing,
                                                              lastAttributesForPercent.frame.origin.y,
                                                              itemSize.width,
                                                              itemSize.height);
                                // 再添加进计算比例的数组
                                [arrayOfPercent
                            ...
                            if (i == itemCount - 1) {
                                ...
                                for (NSInteger i = 0; i < arrayOfPercent.count; i++) {
                                    ...
                                    newAttributes.frame
                                        = CGRectMake(itemX, (maxYOfPercent == -1) ? y : maxYOfPercent + minimumLineSpacing, realWidth * [dic[@"percent"] floatValue], newAttributes.frame.size.height);
                                    newAttributes.indexPath = dic[@"indexPath"];
                                    [self.attributesArray addObject:newAttributes];
                                }
                                for (NSInteger i = 0; i < arrayOfPercent.count; i++) {
...
#pragma mark - 填充布局处理
                    case HDCollectionLayoutTypeFill: {
                        BOOL qualified = YES;
                        if (arrayOfFill.count == 0) {
                            attributes.frame = CGRectMake(self.isFloor ? floor(edgeInsets.left) : edgeInsets.left,
                                                          self.isFloor ? floor(maxYOfFill) : maxYOfFill,
                                                          self.isFloor ? floor(itemSize.width) : itemSize.width,
                                                          self.isFloor ? floor(itemSize.height) : itemSize.height);
                            ...
                            for (NSNumber *yFill in arrayYOfFill) {
                                for (NSNumber *xFill in arrayXOfFill) {
                                    ...
                                    CGFloat attrY = floor([yFill floatValue]) == floor(maxYOfFill) ? (self.isFloor ? floor([yFill floatValue]) : floor([yFill floatValue])) :
                                                                                                     floor([yFill floatValue]) + floor(minimumLineSpacing);
                                    ...
                                    for (HDCollectionViewLayoutAttributes *attr in arrayOfFill) {
                        ...
                        if (qualified == YES) {
                            ...
                            if (![arrayXOfFill
                                    containsObject:self.isFloor ? @(floor(attributes.frame.origin.x + attributes.frame.size.width)) : @(attributes.frame.origin.x + attributes.frame.size.width)]) {
                            ...
                            if (![arrayYOfFill
                                    containsObject:self.isFloor ? @(floor(attributes.frame.origin.y + attributes.frame.size.height)) : @(attributes.frame.origin.y + attributes.frame.size.height)]) {
                                [arrayYOfFill
                                    addObject:self.isFloor ? @(floor(attributes.frame.origin.y + attributes.frame.size.height)) : @(attributes.frame.origin.y + attributes.frame.size.height)];
                            }
...
#pragma mark - 绝对定位布局处理
                    case HDCollectionLayoutTypeAbsolute: {
                ...
                attributes.indexPath = indexPath;
                if (self.layoutType != HDCollectionLayoutTypePercent) {
                ...
                if (self.layoutType == HDCollectionLayoutTypeClosest) {
                    ...
                    lastY = max;
                } else if (self.layoutType == HDCollectionLayoutTypePercent) {
                    lastY = maxYOfPercent;
                } else if (self.layoutType == HDCollectionLayoutTypeFill) {
                    if (i == itemCount - 1) {
                        for (HDCollectionViewLayoutAttributes *attr in arrayOfFill) {
                    ...
                    lastY = maxYOfFill;
                } else if (self.layoutType == HDCollectionLayoutTypeAbsolute) {
                    if (i == itemCount - 1) {
                        for (HDCollectionViewLayoutAttributes *attr in arrayOfAbsolute) {
        ...
        if (footerH > 0) {
            ...
            HDCollectionViewLayoutAttributes *footerAttr = [HDCollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionFooter
...
#pragma mark - 添加背景图
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(collectionView:layout:registerBackView:)]) {
            NSString *className = [self.delegate collectionView:self.collectionView layout:self registerBackView:index];
            if (className != nil && className.length > 0) {
                HDCollectionViewBackgroundViewLayoutAttributes *attr =
                    [HDCollectionViewBackgroundViewLayoutAttributes layoutAttributesForDecorationViewOfKind:className withIndexPath:[NSIndexPath indexPathForRow:0 inSection:index]];
                ...
                HDCollectionViewLayoutAttributes *attr = [HDCollectionViewLayoutAttributes layoutAttributesForDecorationViewOfKind:@"HDCollectionReusableView"
                                                                                                                     withIndexPath:[NSIndexPath indexPathForRow:0 inSection:index]];
                attr.frame = CGRectMake(0, [self isAttachToTop:index] ? itemStartY - headerH : itemStartY, self.collectionView.frame.size.width, backHeight);
    ...
    //    for (HDCollectionViewLayoutAttributes* attr in self.attributesArray) {
