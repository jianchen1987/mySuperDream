///
//  HDCollectionViewBaseFlowLayout.m
//  SuperApp
//
...
#import "HDCollectionCellFakeView.h"
#import "HDCollectionViewLayoutAttributes.h"

typedef NS_ENUM(NSUInteger, HDCollectionViewScrollDirection) {
    HDCollectionViewScrollDirectionStay,
    HDCollectionViewScrollDirectionToTop,
    HDCollectionViewScrollDirectionToEnd,
};


@interface HDCollectionViewBaseFlowLayout () <UIGestureRecognizerDelegate>

/// 关于拖动的参数
@property (nonatomic, strong) HDCollectionCellFakeView *cellFakeView;
@property (nonatomic, strong) UILongPressGestureRecognizer *longPress;
@property (nonatomic, strong) UIPanGestureRecognizer *panGesture;
@property (nonatomic, assign) CGPoint fakeCellCenter;
@property (nonatomic, assign) CGPoint panTranslation;
@property (nonatomic) HDCollectionViewScrollDirection continuousScrollDirection;
@property (nonatomic, strong) CADisplayLink *displayLink;

@end


@implementation HDCollectionViewBaseFlowLayout {
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.layoutType = HDCollectionLayoutTypeFill;
        self.columnCount = 1;
            ...
            for (UICollectionViewLayoutAttributes *attriture in self.headerAttributesArray) {
                ...
                if (section == 0) {
                    if (self.scrollDirection == UICollectionViewScrollDirectionVertical) {
                        CGFloat offsetY = self.collectionView.contentOffset.y + self.headerOffsetY;
                        ...
                        CGFloat offsetX = self.collectionView.contentOffset.y + self.headerOffsetY;
                    ...
                    if (self.scrollDirection == UICollectionViewScrollDirectionVertical) {
                        CGFloat offsetY = self.collectionView.contentOffset.y + self.headerOffsetY;
                        if (offsetY > [self.collectionHeightsArray[section - 1] floatValue] && offsetY < [self.collectionHeightsArray[section] floatValue]) {
                            frame.origin.y = offsetY;
                            attriture.zIndex = 1000 + section;
                            attriture.frame = frame;
                            isNeedChangeFrame = YES;
                        ...
                        CGFloat offsetX = self.collectionView.contentOffset.y + self.headerOffsetY;
                        if (offsetX > [self.collectionHeightsArray[section - 1] floatValue] && offsetX < [self.collectionHeightsArray[section] floatValue]) {
                            frame.origin.x = offsetX;
                            attriture.zIndex = 1000 + section;
                            attriture.frame = frame;
                            isNeedChangeFrame = YES;
                ...
                if (!isNeedChangeFrame) {
                    /*
                      ...
                      否则header无法正确归位
                     */
                    if ([attriture isKindOfClass:[HDCollectionViewLayoutAttributes class]]) {
                        attriture.frame = ((HDCollectionViewLayoutAttributes *)attriture).orginalFrame;
                    }
                }
            }
        }
...
- (void)setUpGestureRecognizers {
    ...
    __weak typeof(HDCollectionViewBaseFlowLayout *) weakSelf = self;
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)longPress {
    ...
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            self.cellFakeView = [[HDCollectionCellFakeView alloc] initWithCell:currentCell];
...
- (void)beginScrollIfNeeded {
    ...
    if (fakeCellTopEdge <= offset + paddingTop + trigerInsetTop) {
        self.continuousScrollDirection = HDCollectionViewScrollDirectionToTop;
        [self setUpDisplayLink];
    } else if (fakeCellEndEdge >= offset + length - paddingEnd - trigerInsetEnd) {
        self.continuousScrollDirection = HDCollectionViewScrollDirectionToEnd;
        [self setUpDisplayLink];
    } else {
        [self invalidateDisplayLink];
    }
...
- (void)moveItemIfNeeded {
    ...
    for (HDCollectionViewLayoutAttributes *attr in weakSelf.attributesArray) {
        if (attr.indexPath.section == toIndexPath.section && attr.indexPath.item == toIndexPath.item && attr.representedElementKind != UICollectionElementKindSectionHeader
...
- (void)invalidateDisplayLink {
    _continuousScrollDirection = HDCollectionViewScrollDirectionStay;
...
- (CGFloat)calcTrigerPercentage {
    ...
    if (self.continuousScrollDirection == HDCollectionViewScrollDirectionToTop) {
        if (self.cellFakeView) {
            percentage = 1.0 - ((self.cellFakeView.frame.origin.y - (offset + paddingTop)) / trigerInsetTop);
        }
    } else if (self.continuousScrollDirection == HDCollectionViewScrollDirectionToEnd) {
        if (self.cellFakeView) {
...
- (CGFloat)scrollValueWithSpeed:(CGFloat)speed andPercentage:(CGFloat)percentage {
    ...
    switch (_continuousScrollDirection) {
        case HDCollectionViewScrollDirectionStay: {
            return 0.0f;
            break;
        }
        case HDCollectionViewScrollDirectionToTop: {
            value = -speed;
            break;
        }
        case HDCollectionViewScrollDirectionToEnd: {
            value = speed;
            break;
        }
        default: {
