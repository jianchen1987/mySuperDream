...
@interface WMHomeViewModel () <HDResponseDelegate>
/// 附近商家标题占位
...
@implementation WMHomeViewModel
...
- (void)addColectionData:(WMHomeLayoutModel *)obj {
    ...
    if (([obj.identity isEqualToString:WMHomeLayoutTypeFoucsAdvertise] || [obj.identity isEqualToString:WMHomeLayoutTypeCarouselAdvertise] ||
        ...
        [array enumerateObjectsUsingBlock:^(WMAdadvertisingModel *_Nonnull obj1, NSUInteger idx, BOOL *_Nonnull stop) {
            ...
            [LKDataRecord.shared traceEvent:obj.event[@"name"] name:obj.event[@"name"] parameters:mdics SPM:[LKSPM SPMWithPage:@"WMHomeViewController" area:@"" node:@""]];
        }];
    } else {
        NSMutableDictionary *mdics = NSMutableDictionary.new;
        if (plateId)
...
- (void)getApolloLayoutConfigGetLocal:(BOOL)getLocal completion:(void (^)(id json))completion {
    ...
    NSTimeInterval oldTime = [[[NSUserDefaults standardUserDefaults] objectForKey:@"yunnow_home_layout_config"] doubleValue];
    /// 10分钟重新请求
    BOOL reRequest = NO;
    if (currentTime - oldTime >= 10 * 60) {
        reRequest = YES;
    }
    NSArray<WMHomeLayoutModel *> *resultInfo = [SACacheManager.shared objectForKey:kCacheKeyHomeLayoutApollo type:SACacheTypeDocumentPublic];
    if (resultInfo && (getLocal || !reRequest)) {
        !completion ?: completion(resultInfo);
        return;
    }
    [self.storeDTO getWMAplioConfigSuccess:^(id _Nonnull rspModel) {
        !completion ?: completion(rspModel);
        [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%lf", NSDate.date.timeIntervalSince1970] forKey:@"yunnow_home_layout_config"];
    } failure:^(SARspModel *_Nullable rspModel, CMResponseErrorType errorType, NSError *_Nullable error) {
        !completion ?: completion(resultInfo);
    }];
}
...
- (NSArray<WMHomeLayoutModel *> *)getDefaultLayoutJSON:(id)json {
    ...
    if (!json) {
        fromLocal = true;
        json = [NSJSONSerialization JSONObjectWithData:[NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"homeLayout" ofType:@"json"]] options:NSJSONReadingMutableLeaves
                                                 error:nil];
    }
    ...
    if (!fromLocal) {
        [SACacheManager.shared setObject:resultInfo forKey:kCacheKeyHomeLayoutApollo type:SACacheTypeDocumentPublic];
    }
    ...
    if (!_layoutInfo) {
        _layoutInfo = @{
            ...
            WMHomeLayoutTypeFoucsAdvertise: @{
                @"cellClass": @"WMFoucsAdvertiseTableViewCell",
                @"URL": @"/takeaway-merchant/app/super-app/get-advertising",
                @"layoutConfig": @{
                    @"outSets": [NSValue valueWithUIEdgeInsets:UIEdgeInsetsMake(0, 0, kRealWidth(16), 0)],
            ...
            WMHomeLayoutTypeKingKong: @{
                @"cellClass": @"WMKingKongTableViewCell",
            ...
            WMHomeLayoutTypeEatOnTime: @{
                @"cellClass": @"WMThemeTableViewCell",
            ...
            WMHomeLayoutTypeCarouselAdvertise: @{
                @"cellClass": @"WMCarouselAdvertiseTableViewCell",
                @"URL": @"/takeaway-merchant/app/super-app/get-advertising",
                @"layoutConfig": @{
                    @"outSets": [NSValue valueWithUIEdgeInsets:UIEdgeInsetsMake(0, kRealWidth(12), kRealWidth(20), kRealWidth(12))],
            ...
            WMHomeLayoutTypeNearStoreAdvertise: @{
                @"cellClass": @"WMNearStoreAdvertiseTableViewCell",
                @"URL": @"/takeaway-merchant/app/super-app/get-advertising",
                @"layoutConfig": @{
                    @"outSets": [NSValue valueWithUIEdgeInsets:UIEdgeInsetsMake(kRealWidth(20), kRealWidth(12), kRealWidth(20), kRealWidth(12))],
                    @"inSets": [NSValue valueWithUIEdgeInsets:UIEdgeInsetsZero],
                    @"contenInset": [NSValue valueWithUIEdgeInsets:UIEdgeInsetsZero],
                },
