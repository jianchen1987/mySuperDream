...
@interface WMSpecialActivesProductView ()
/// offset
...
@implementation WMSpecialActivesProductView
...
- (void)updateHeadView {
    @HDWeakify(self) self.headView.frameBlock = ^(CGFloat realHeight) {
        @HDStrongify(self) if (self.hidden) return;
        if ([self.viewModel.type isEqualToString:WMSpecialActiveTypeEat]) {
            self.headView.frame = CGRectMake(0, -(realHeight - kRealWidth(30)), self.headView.size.width, realHeight);
            self.collectionView.contentInset = UIEdgeInsetsMake(realHeight - kRealWidth(30), 0, 0, 0);
        } else {
            CGFloat height = realHeight + kRealWidth(20);
            self.headView.frame = CGRectMake(0, -height, kScreenWidth, height - kRealWidth(20));
        ...
        self.viewModel.imageHeight = self.collectionView.contentInset.top;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    WMSpecialActivesProductCollectionViewCell *cell = [WMSpecialActivesProductCollectionViewCell cellWithCollectionView:collectionView forIndexPath:indexPath];
...
- (SACollectionView *)collectionView {
    if (!_collectionView) {
        ...
        _collectionView.backgroundColor = HDAppTheme.color.G5;
